//Bezverkhaia Karina 2271100, avg+extra task
#include <glad/glad.h>
...
// Toggle State Flag for extra credit
...
glm::mat4 objectScale = glm::scale(glm::mat4(1.0f), glm::vec3(0.08f));
glm::mat4 objectFloorX = glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)) * objectScale;
...
void myDisplay()
{
	...
	//Shoulder
	glm::mat4 shoulder = glm::translate(model, glm::vec3(0.0f, 0.35f, 0.0f));
	...
	//Fingers
	glm::mat4 leftFinger = glm::translate(wrist, glm::vec3(0.0f, 0.20f, 0.0f));
	leftFinger = glm::rotate(leftFinger, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(leftFinger);

	glm::mat4 rightFinger = glm::translate(wrist, glm::vec3(0.0f, 0.20f, 0.0f));
	rightFinger = glm::rotate(rightFinger, glm::radians(45.0f + FingerAng2), glm::vec3(0.0f, 0.0f, 0.1f));
	DrawFingerBase(rightFinger);

	//Tips
	glm::mat4 leftTip = glm::translate(leftFinger, glm::vec3(0.0f, 0.35f, 0.0f));
	leftTip = glm::rotate(leftTip, glm::radians(-(45.0f + FingerAng1)), glm::vec3(0.0f, 0.0f, 0.4f));
	DrawFingerTip(leftTip);

	glm::mat4 rightTip = glm::translate(rightFinger, glm::vec3(0.0f, 0.35f, 0.0f));
	rightTip = glm::rotate(rightTip, glm::radians(-(3.0f + FingerAng2)), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(rightTip);

	//For extra credit comments
	//Get world positions of both tips
	glm::vec3 pL = glm::vec3(leftTip[3]);
	glm::vec3 pR = glm::vec3(rightTip[3]);
	//midpoint between tips
	glm::vec3 mid = (pL + pR) * 0.5f;

	glm::mat3 wristRot = glm::mat3(wrist);

	if (FollowTeapot){
		// Put the teapot at the midpoint. Only translation + uniform scale are used
		glm::mat4 T = glm::translate(glm::mat4(1.0f), mid);
		DrawObject(T * objectScale);
	}
	else 
	{
		// Keep the teapot at the original floor position
		DrawObject(objectFloorX);
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// press space button for extra credit
