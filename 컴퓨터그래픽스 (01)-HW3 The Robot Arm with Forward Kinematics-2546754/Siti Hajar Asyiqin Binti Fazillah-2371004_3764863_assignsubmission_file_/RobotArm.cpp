...
// HW3: The Robot Arm with Forward Kinematics
// extra credit question included (refer to comments)
...
float FingerAng2 = -90;
bool isTouching = false;   // space (extra credit)
...
// (extra credit)
// track current world transformation matrix of the teapot - updated to keep it position in the world
glm::mat4 teapotWorldTransform = objectXform;
// offset calculated when teapot is 'grabbed' - to maintain correct relative positioning
glm::mat4 teapotLocalOffset = glm::mat4(1.0f);
// global transformation matrix of left finger tip (used as a reference frame for space key action)
glm::mat4 fingerTipLeftGlobal = glm::mat4(1.0f);
...
void myDisplay()
{
	...
	// ROBOT RENDERING STUFF
	// BASE > SHOULDER > ELBOW > WRIST > LEFT BASE, TIP > RIGHT BASE, TIP
	
	// matrix multiplication is not commutative (most right transformation is applied first)
	// start base model matrix > translate joint position > rotate around new origin
	...
	DrawBase(baseModel);
	// no need to explicitly call DrawJoint() as it is already called in DrawBase(), DrawArmSegment()
	// if still explicitly called? - override or extra visualisation
	// DrawJoint(baseModel);

	glm::mat4 shoulderModel = glm::translate(baseModel, glm::vec3(0.0f, 0.425f, 0.0f));
	shoulderModel = glm::rotate(shoulderModel, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(shoulderModel);

	glm::mat4 elbowModel = glm::translate(shoulderModel, glm::vec3(0.0f, 0.5f, 0.0f));
	...
	glm::mat4 fingerBaseLeft = glm::translate(wristModel, glm::vec3(0.03f, 0.2f, 0.0f));
	fingerBaseLeft = glm::rotate(fingerBaseLeft, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(fingerBaseLeft);

	glm::mat4 fingerTipLeft = glm::translate(fingerBaseLeft, glm::vec3(0.0f, 0.35f, 0.0f));
	fingerTipLeft = glm::rotate(fingerTipLeft, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(fingerTipLeft);

	// used for (extra credit question)
	fingerTipLeftGlobal = fingerTipLeft; // latest computed fingerTipLeft matrix (current world position)

	glm::mat4 fingerBaseRight = glm::translate(wristModel, glm::vec3(-0.03f, 0.2f, 0.0f));
	fingerBaseRight = glm::rotate(fingerBaseRight, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(fingerBaseRight);

	glm::mat4 fingerTipRight = glm::translate(fingerBaseRight, glm::vec3(0.0f, 0.35f, 0.0f));
	fingerTipRight = glm::rotate(fingerTipRight, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(fingerTipRight);

	// (extra credit +50%)
	// Use the space bar to toggle whether the yellow teapot
	// follows the motion of the wrist joint or not.
	if (isTouching) {
		// calculate teapot world transform (apply local offset relative to fingertip)
		// == teapot follow finger's movement
		glm::mat4 teapotAtFinger = fingerTipLeftGlobal * teapotLocalOffset;
		DrawObject(teapotAtFinger);
	}
	else {
		// when not 'grabbing', draw teapot at its last known world transform
		// so it stays in place after being released
		DrawObject(teapotWorldTransform);
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// (extra credit - space key pressed)
	// hint given by prof - to use 역행렬 concept (inverse matrix)
	else if (key == GLFW_KEY_SPACE && action == GLFW_PRESS)
	{
		isTouching = !isTouching;

		// use inverse matrix to switch between world and local coordinate frames
		if (isTouching) {
			glm::mat4 fingerWorld = fingerTipLeftGlobal;
			// teapotLocalOffset as the inverse of the fingertip transfrom multiplied by current teapot world transform
			// can move with the finger while maintain relative pose
			teapotLocalOffset = glm::inverse(fingerTipLeftGlobal) * teapotWorldTransform;
			// M^{-1} * (M * v) = v // M transformation matrix (inverse), v point
		}
		else
		{
			glm::mat4 fingerWorld = fingerTipLeftGlobal;
			// update by multiplying the fingertip transform by the stored local offset
			teapotWorldTransform = fingerTipLeftGlobal * teapotLocalOffset;
		}
	}
}
