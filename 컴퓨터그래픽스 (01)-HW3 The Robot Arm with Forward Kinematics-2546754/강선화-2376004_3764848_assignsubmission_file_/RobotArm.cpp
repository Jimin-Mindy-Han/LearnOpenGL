...
float FingerAng2 = -45;
float FingerTipAng1 = -90;
float FingerTipAng2 = 90;
...
// teapot이 로봇팔을 따라 움직일지 확인하기 위한 변수
bool teapotFollowReq = false;
bool teapotFollow = false;


// 초기 위치
glm::mat4 teapot = objectXform;

// teapot 위치 계산하기 위한 변수
glm::mat4 teapotOffset = glm::mat4(1.0f);
...
void myDisplay()
{
	...
	// base
	glm::mat4 base = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	...
	// left finger
	glm::mat4 leftFinger = glm::translate(wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	leftFinger = glm::rotate(leftFinger, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(leftFinger);


	// right finger
	glm::mat4 rightFinger = glm::translate(wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	rightFinger = glm::rotate(rightFinger, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerBase(rightFinger);


	// left fingertip
	glm::mat4 leftFingerTip = glm::translate(leftFinger, glm::vec3(0.05f, 0.35f, 0.0f));
	leftFingerTip = glm::rotate(leftFingerTip, glm::radians(FingerTipAng1), glm::vec3(0, 0, 1));
	DrawFingerTip(leftFingerTip);


	//right fingertip
	glm::mat4 rightFingerTip = glm::translate(rightFinger, glm::vec3(-0.05f, 0.35f, 0.0f));
	rightFingerTip = glm::rotate(rightFingerTip, glm::radians(FingerTipAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(rightFingerTip);

	// extra
	// 스페이스바 눌렀을 때
	if (teapotFollowReq == true) {
		if (teapotFollow == false) {
			// 현재 teapot 위치와 로봇팔 사이 거리 계산
			teapotOffset = glm::inverse(wrist) * teapot;
			teapotFollow = true;
		} else {
			// 더이상 따라서 움직이지 않음
			teapotFollow = false;
		...
		teapotFollowReq = false;
	}

	if (teapotFollow == true)
		teapot = wrist * teapotOffset;
