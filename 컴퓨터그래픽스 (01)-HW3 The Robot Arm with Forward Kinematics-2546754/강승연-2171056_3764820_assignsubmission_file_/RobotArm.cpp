...
glm::mat4 GetXform(glm::mat4 coordinate, glm::vec3  position, float angle, glm::vec3 w) {
	glm::mat4 base = glm::mat4(1.0f);
	base = coordinate * glm::translate(base, position);
	base = glm::rotate(base, glm::radians(angle), w);
	return base;
}
glm::mat4 GetXformWithRotationTwice(glm::mat4 coordinate, glm::vec3  position, float angle1, glm::vec3 w1, float angle2, glm::vec3 w2) {
	glm::mat4 base = glm::mat4(1.0f);
	base = coordinate * glm::translate(base, position);
	base = glm::rotate(base, glm::radians(angle1), w1);
	base = glm::rotate(base, glm::radians(angle2), w2);
	return base;
}
...
bool MoveTeapot = false;
glm::mat4 robotCoordinate; //로봇팔 기준 좌표계 = wrist
glm::mat4 localXform; //로봇팔 기준 좌표계로 본 teapot의 상대좌표
...
void myDisplay()
{
	...
	glm::mat4 base = GetXform(glm::mat4(1.0f), glm::vec3(BaseTransX, 0.0f, BaseTransZ), BaseSpin, yAxis);
	glm::mat4 shoulder = GetXform(base, glm::vec3(0.0f, 0.4f, 0.0f), ShoulderAng, zAxis);
	glm::mat4 elbow = GetXform(shoulder, glm::vec3(0.0f, 0.5f, 0.0f), ElbowAng, zAxis);
	glm::mat4 wrist = GetXformWithRotationTwice(elbow, glm::vec3(0.0f, 0.5f, 0.0f), WristAng, zAxis, WristTwistAng, yAxis);
	glm::mat4 fingerBase1 = GetXform(wrist, glm::vec3(0.0f, 0.2f, 0.0f), FingerAng1, zAxis);
	glm::mat4 fingerBase2 = GetXform(wrist, glm::vec3(0.0f, 0.2f, 0.0f), -FingerAng1, zAxis);
	glm::mat4 fingerTip1 = GetXform(fingerBase1, glm::vec3(0.0f, 0.35f, 0.0f), FingerAng2, zAxis);
	glm::mat4 fingerTip2 = GetXform(fingerBase2, glm::vec3(0.0f, 0.35f, 0.0f), -FingerAng2, zAxis);

	robotCoordinate = wrist;
	...
	if (MoveTeapot == true) {
		// 스페이스바를 눌렀을 때, wrist 좌표계 기준으로 obj를 서술한 값을 localXform에 저장한다 => 인풋 코드에서 처리(processInput 함수 추가 수정)
		// wrist 좌표계에서 world 좌표계로 돌려놓은 값을 objXform에 저장한다
		objectXform = robotCoordinate * localXform;
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// Extra Credit
		...
		localXform = glm::inverse(robotCoordinate) * objectXform;
	}
}
