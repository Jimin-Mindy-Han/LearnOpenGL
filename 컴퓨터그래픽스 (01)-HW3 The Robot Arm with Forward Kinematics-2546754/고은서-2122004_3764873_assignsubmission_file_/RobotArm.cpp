// Extra credit (+50%) code implemented
// -- Use the space bar to toggle whether the yellow teapot follows the motion of the wrist joint or not. 
// 티팟이 손목을 따라가게 하는 코드 구현 완료했습니다.
...
// teapot follows wrist
bool SpaceBarPressed = false; // SPACE BAR STUFF
glm::mat4 WristPrev = glm::mat4(1.0f);
bool WristPrevValid = false;
...
void myDisplay()
{
	...
	// M0 = DrawBase(model)
	glm::mat4 M0 = model;
	M0 = glm::translate(M0, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	M0 = glm::rotate(M0, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(M0);

	// M1 = DrawArmSegment(M0) : upper arm
	glm::mat4 M1 = M0;
	M1 = glm::translate(M1, glm::vec3(0.0f, 0.4f, 0.0f));
	M1 = glm::rotate(M1, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(M1);

	// M2 = DrawArmSegment(M1) : lower arm
	glm::mat4 M2 = M1;
	M2 = glm::translate(M2, glm::vec3(0.0f, 0.5f, 0.0f));
	M2 = glm::rotate(M2, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(M2);

	// M3 = DrawWrist(M2) : wrist
	glm::mat4 M3 = M2;
	M3 = glm::translate(M3, glm::vec3(0.0f, 0.5f, 0.0f));
	M3 = glm::rotate(M3, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	M3 = glm::rotate(M3, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(M3);

	// DrawFingerPair(M3) : fingers
	glm::mat4 fingerBase = glm::translate(M3, glm::vec3(0.0f, 0.2f, 0.0f));
	const float halfGap = 0.06f;

	// left finger
	glm::mat4 leftBase = glm::translate(fingerBase, glm::vec3(+halfGap, 0.0f, 0.0f));
	leftBase = glm::rotate(leftBase, glm::radians(+FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(leftBase);

	glm::mat4 leftTip = glm::translate(leftBase, glm::vec3(0.0f, 0.35f, 0.0f));
	leftTip = glm::rotate(leftTip, glm::radians(+FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(leftTip);

	// right finger
	glm::mat4 rightBase = glm::translate(fingerBase, glm::vec3(-halfGap, 0.0f, 0.0f));
	rightBase = glm::rotate(rightBase, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(rightBase);

	glm::mat4 rightTip = glm::translate(rightBase, glm::vec3(0.0f, 0.35f, 0.0f));
	rightTip = glm::rotate(rightTip, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(rightTip);

	// teapot follows wrist : delta = M3_now * inverse(M3_prev)
	if (SpaceBarPressed) {
		if (!WristPrevValid) {
			WristPrev = M3;
			WristPrevValid = true;
		}
		glm::mat4 delta = M3 * glm::inverse(WristPrev);
		objectXform = delta * objectXform;
		WristPrev = M3;
	}
	else {
		WristPrevValid = false;
	}
