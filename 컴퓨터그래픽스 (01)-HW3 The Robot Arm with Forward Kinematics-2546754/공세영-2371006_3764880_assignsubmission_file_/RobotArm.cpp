...
int RobotControl = 0;
bool isSpaceOn = false;	// Space가 눌렸는지, wrist를 따라가야 하는지 여부 저장
glm::mat4 teapotTransform = glm::mat4(1.0f); // 다병의 transform을 저장
glm::mat4 wristGlobal = glm::mat4(1.0f);      // wrist의 현재 transform 저장
glm::mat4 wristReference = glm::mat4(1.0f);   // 스페이스 키를 누른 시점의 wrist transform 저장
glm::mat4 objectOffset = glm::mat4(1.0f);     // wristReference에 대한 다병 오브젝트의 상대적인 transform 저장
...
void myDisplay()
{
	...
	glm::mat4 shoulder = glm::translate(robotBase, glm::vec3(0.0f, 0.4f, 0.0f));
	...
	glm::mat4 fingerBase = glm::translate(wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	glm::mat4 leftFinger = glm::translate(fingerBase, glm::vec3(0.05f, 0.0f, 0.0f));
	leftFinger = glm::rotate(leftFinger, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(leftFinger);
	glm::mat4 leftFingerTip = glm::translate(leftFinger, glm::vec3(0.0f, 0.35f, 0.0f)); // 0.4f -> 0.35f
	leftFingerTip = glm::rotate(leftFingerTip, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(leftFingerTip);

	glm::mat4 rightFinger = glm::translate(fingerBase, glm::vec3(-0.05f, 0.0f, 0.0f));
	rightFinger = glm::rotate(rightFinger, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(rightFinger);
	glm::mat4 rightFingerTip = glm::translate(rightFinger, glm::vec3(0.0f, 0.35f, 0.0f)); // 0.4f -> 0.35f
	rightFingerTip = glm::rotate(rightFingerTip, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(rightFingerTip);

	if (isSpaceOn)
	{
		glm::mat4 delta = wristGlobal * glm::inverse(wristReference);
		objectXform = delta * wristReference * objectOffset;
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_SPACE && action == GLFW_PRESS)	// 스페이스 누른 경우
	{
		isSpaceOn = !isSpaceOn;	// 스페이스 키 온오프 토글을 시켜준다.
		if (isSpaceOn)
		{
			wristReference = wristGlobal;	// 스페이스 키를 누른 시점의 wrist transform 저장
			objectOffset = glm::inverse(wristReference) * objectXform; // wristReference에 대한 다병 오브젝트의 상대적인 transform 저장
		}
	}
}
