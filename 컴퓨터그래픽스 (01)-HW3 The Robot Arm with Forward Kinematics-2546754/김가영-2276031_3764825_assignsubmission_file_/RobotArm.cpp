...
// FOR EXTRA CREDIT
bool ObjectFollowsWrist = false; // 주전자 추적 상태
...
// extra credit
glm::mat4 TeapotCurrentXform = objectXform;

glm::mat4 FollowRef_Grip = glm::mat4(1.0f); 
glm::mat4 FollowRef_Object = objectXform;      // O(ton)

glm::mat4 gGripWorld = glm::mat4(1.0f);

glm::mat4 TeapotRelativeOffset = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.1f, 0.0f)), glm::vec3(0.08f, 0.08f, 0.08f));
...
void myDisplay()
{
	...
	// Base
	glm::mat4 Base = glm::translate(glm::mat4(1.0f), glm::vec3(BaseTransX, 0.025f, BaseTransZ));
	Base = glm::rotate(Base, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(Base);

	// Shoulder
	glm::mat4 Shoulder = glm::translate(Base, glm::vec3(0.0f, 0.4f, 0.0f));
	Shoulder = glm::rotate(Shoulder, glm::radians(ShoulderAng), glm::vec3(1.0f, 0.0f, 1.0f)); 
	DrawArmSegment(Shoulder);

	// Elbow
	glm::mat4 Elbow = glm::translate(Shoulder, glm::vec3(0.0f, 0.5f, 0.0f));
	Elbow = glm::rotate(Elbow, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(Elbow);

	// Wrist
	glm::mat4 Wrist = glm::translate(Elbow, glm::vec3(0.0f, 0.5f, 0.0f));
	...
	glm::mat4 GripAnchor = glm::translate(Wrist, glm::vec3(0.0f, 0.2f, 0.0f));

	// --- Finger 1 Base ---
	glm::mat4 Finger1Base = glm::translate(Wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	Finger1Base = glm::rotate(Finger1Base, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(Finger1Base);

	gGripWorld = GripAnchor; 

	// Finger 1 Tip
	glm::mat4 Finger1Tip = glm::translate(Finger1Base, glm::vec3(0.0f, 0.35f, 0.0f));
	Finger1Tip = glm::rotate(Finger1Tip, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(Finger1Tip);

	// --- Finger 2 Base  ---
	glm::mat4 Finger2Base = glm::translate(Wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	Finger2Base = glm::rotate(Finger2Base, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));

	Finger2Base = glm::rotate(Finger2Base, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f)); 

	DrawFingerBase(Finger2Base);

	// Finger 2 Tip
	glm::mat4 Finger2Tip = glm::translate(Finger2Base, glm::vec3(0.0f, 0.35f, 0.0f));
	Finger2Tip = glm::rotate(Finger2Tip, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(Finger2Tip);

	// =========================================================================
// === Extra Credit ===
	if (ObjectFollowsWrist) {
		// O(t) = W(t) * inverse(W_on) * O_on
		TeapotCurrentXform = gGripWorld * glm::inverse(FollowRef_Grip) * FollowRef_Object;
	}
	DrawObject(TeapotCurrentXform);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		ObjectFollowsWrist = !ObjectFollowsWrist;

		if (ObjectFollowsWrist)
		{
			FollowRef_Grip = gGripWorld;  // W(ton)
			FollowRef_Object = TeapotCurrentXform; // O(ton)
		}
	}
}
