...
void myDisplay()
{
	...
	// baseModel
	// 1번키 -> 로봇 전체 위치 이동 
	glm::mat4 baseModel = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	baseModel = glm::rotate(baseModel, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f)); //y축 회전 (2번키 누르면)
	DrawBase(baseModel);
	
	// armSegmentModel
	glm::mat4 armSegmentModel = glm::translate(baseModel, glm::vec3(0.0f, 0.45f, 0.0f)); // base에 연결
	armSegmentModel = glm::rotate(armSegmentModel, glm::radians(ShoulderAng), glm::vec3(1.0f, 0.0f, 0.0f)); // x축 회전 (3번키 누르면)
	DrawArmSegment(armSegmentModel);

	// armSegmentModel2
	glm::mat4 armSegmentModel2 = glm::translate(armSegmentModel, glm::vec3(0.0f, 0.5f, 0.0f)); // armSegmentModel에 연결
	armSegmentModel2 = glm::rotate(armSegmentModel2, glm::radians(ElbowAng), glm::vec3(1.0f, 0.0f, 0.0f)); // (3번키 누르면 회전)
	DrawArmSegment(armSegmentModel2);

	// wristModel
	glm::mat4 wristModel = glm::translate(armSegmentModel2, glm::vec3(0.0f, 0.5f, 0.0f));  // armSegmentModel2에 연결
	wristModel = glm::rotate(wristModel, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f)); // (4번키 누르면 회전)
	wristModel = glm::rotate(wristModel, glm::radians(WristAng), glm::vec3(1.0f, 0.0f, 0.0f));  // (4번키 누르면 회전)
	DrawWrist(wristModel);

	// 아래는 5번키 누르면 회전
	// fingerBaseModel1 (한쪽 fingerbase)
	glm::mat4 fingerBaseModel1 = glm::translate(wristModel, glm::vec3(0.0f, 0.2f, 0.0f)); //wristModel에 연결
	fingerBaseModel1 = glm::rotate(fingerBaseModel1, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f));
	DrawFingerBase(fingerBaseModel1);

	// fingerBaseModel2 (다른쪽 fingerbase)
	glm::mat4 fingerBaseModel2 = glm::translate(wristModel, glm::vec3(0.0f, 0.2f, 0.0f));  //wristModel에 연결
	fingerBaseModel2 = glm::rotate(fingerBaseModel2, glm::radians(-FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f)); // fingerBaseModel1과 반대방향으로 회전하도록 
	DrawFingerBase(fingerBaseModel2);

	// fingerTipModel1 (fingerBaseModel1에 연결된 쪽)
	glm::mat4 fingerTipModel1 = glm::translate(fingerBaseModel1, glm::vec3(0.0f, 0.35f, 0.0f)); //fingerBaseModel1에 연결
	fingerTipModel1 = glm::rotate(fingerTipModel1, glm::radians(FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f));
	DrawFingerTip(fingerTipModel1);

	// fingerTipModel2 (fingerBaseModel2에 연결된 쪽)
	glm::mat4 fingerTipModel2 = glm::translate(fingerBaseModel2, glm::vec3(0.0f, 0.35f, 0.0f)); //fingerBaseModel2에 연결
	fingerTipModel2 = glm::rotate(fingerTipModel2, glm::radians(-FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f)); // fingerTipModel1과 반대방향으로 회전하도록
