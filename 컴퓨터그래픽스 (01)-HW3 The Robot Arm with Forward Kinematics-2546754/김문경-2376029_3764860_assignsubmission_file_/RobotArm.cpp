...
std::string ourObjectPath = FileSystem::getPath("resources/hw3/teapot.obj");
...
void myDisplay()
{
	...
	glm::mat4 root = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	root = glm::rotate(root, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(root);

	glm::mat4 shoulder = glm::translate(root, glm::vec3(0.0f, 0.4f, 0.0f));
	shoulder = glm::rotate(shoulder, glm::radians(ShoulderAng), glm::vec3(0, 0, 1));
	DrawArmSegment(shoulder);

	glm::mat4 elbow = glm::translate(shoulder, glm::vec3(0.0f, 0.5f, 0.0f));
	elbow = glm::rotate(elbow, glm::radians(ElbowAng), glm::vec3(0, 0, 1));
	DrawArmSegment(elbow);

	glm::mat4 wrist = glm::translate(elbow, glm::vec3(0.0f, 0.5f, 0.0f));
	wrist = glm::rotate(wrist, glm::radians(WristAng), glm::vec3(0, 0, 1));
	wrist = glm::rotate(wrist, glm::radians(WristTwistAng), glm::vec3(0, 1, 0));
	DrawWrist(wrist);

	const float gripX = 0.06f, baseUp = 0.20f, tipUp = 0.35f;

	glm::mat4 fingerL = glm::translate(wrist, glm::vec3(-gripX, baseUp, 0.0f));
	fingerL = glm::rotate(fingerL, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(fingerL);
	glm::mat4 tipL = glm::translate(fingerL, glm::vec3(0.0f, tipUp, 0.0f));
	tipL = glm::rotate(tipL, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(tipL);

	glm::mat4 fingerR = glm::translate(wrist, glm::vec3(+gripX, baseUp, 0.0f));
	fingerR = glm::rotate(fingerR, glm::radians(-FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(fingerR);
	glm::mat4 tipR = glm::translate(fingerR, glm::vec3(0.0f, tipUp, 0.0f));
	tipR = glm::rotate(tipR, glm::radians(-FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(tipR);

	//Extra credit
	const glm::mat4 teapotOffset =
		glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.11f, 0.0f)) * 
		glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(0, 1, 0)) * 
		glm::scale(glm::mat4(1.0f), glm::vec3(0.08f));                 

	glm::mat4 teapotWorld;
	glm::mat4 anchor = tipR;        

	if (followTeapot)
		teapotWorld = anchor * teapotOffset; 
	else
		teapotWorld = objectXform;
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
		...
		followTeapot = !followTeapot;
		std::cout << "followTeapot = " << followTeapot << std::endl;
	}
...
void DrawFingerBase(glm::mat4 model)
{
	glm::mat4 Base = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.15f, 0.0f));
...
void DrawFingerTip(glm::mat4 model)
{
	glm::mat4 Base = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.125f, 0.0f)); // <<-- 수정된 라인
	Base = glm::scale(Base, glm::vec3(0.05f, 0.25f, 0.05f));                         // <<-- 수정된 라인
...
Plane::Plane()
{
	...
	floorTexture = loadTexture(FileSystem::getPath("resources/hw3/wood.png").c_str());
	FloorShader->use();
