...
int RobotControl = 0;
bool SpaceBar = false; // (extra credit) 스페이스바 눌림 여부 저장할 bool 타입 변수
bool flag = false; // 주전자 새 위치 계산 여부
glm::mat4 dist; // 주전자의 바뀐 위치 계산할 행렬
...
void myDisplay()
{
	...
	DrawGroundPlane(model); // 바닥면 그리기

	glm::mat4 model_bottom = glm::mat4(1.0f);
	
	// 1) 로봇팔의 바닥기둥이 마우스 움직임대로 xz 평면에서 translate
	model_bottom = glm::translate(model_bottom, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	
	// 2) 로봇팔이 마우스의 가로 방향 움직임만큼 Y축 기준 rotate
	model_bottom = glm::rotate(model_bottom, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(model_bottom); // 바닥기둥 그리기

	glm::mat4 model_middle = glm::translate(model_bottom, glm::vec3(0.0f, 0.4f, 0.0f));
	// 3) 로봇의 중간기둥이 마우스의 세로 움직임만큼 z축 기준 rotate
	model_middle = glm::rotate(model_middle, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model_middle); // 중간기둥 그리기

	glm::mat4 model_top = glm::translate(model_middle, glm::vec3(0.0f, 0.5f, 0.0f));
	// 3) 로봇의 윗기둥이 마우스의 가로 방향 움직임만큼 Z축 기준rotate
	model_top = glm::rotate(model_top, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model_top); // 윗기둥 그리기

	glm::mat4 model_wrist = glm::translate(model_top, glm::vec3(0.0f, 0.5f, 0.0f));
	// 4) 로봇의 wrist가 마우스의 가로 세로 방향 움직임만큼 y,z축 기준 rotate
	model_wrist = glm::rotate(model_wrist, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	model_wrist = glm::rotate(model_wrist, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(model_wrist); // wrist 그리기

	// 5) 양쪽 fingerbase가 마우스의 세로 방향 움직임만큼 z축 기준 rotate
	glm::mat4 model_FingerBase_left = glm::translate(model_wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	model_FingerBase_left = glm::rotate(model_FingerBase_left, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 model_FingerBase_right = glm::translate(model_wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	model_FingerBase_right = glm::rotate(model_FingerBase_right, -glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(model_FingerBase_left); // FingerBase 그리기
	DrawFingerBase(model_FingerBase_right);

	// 5) 양쪽 fingertip이 마우스의 가로 방향 움직임만큼 z측 기준 rotate
	glm::mat4 model_FingerTip_left = glm::translate(model_FingerBase_left, glm::vec3(0.0f, 0.35f, 0.0f));
	model_FingerTip_left = glm::rotate(model_FingerTip_left, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 model_FingerTip_right = glm::translate(model_FingerBase_right, glm::vec3(0.0f, 0.35f, 0.0f));
	model_FingerTip_right = glm::rotate(model_FingerTip_right, -glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(model_FingerTip_left); // FingerTip 그리기
	DrawFingerTip(model_FingerTip_right);

	// extra credit - 주전자가 wrist 같이 따라 움직이기
	if (SpaceBar) {
		if (flag) {
			dist = inverse(model_wrist) * objectXform; // wrist로부터 주전자가 얼마나 떨어져있는지
			flag = false; // dist는 한 번만 계산하도록
		}		
		objectXform = model_wrist * dist; // 바뀔 주전자 위치. post multiplication
	}
	DrawObject(objectXform); // 주전자 그리기
}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS) // 1~5번 키
		RobotControl = key - GLFW_KEY_1; // 어떤 키 눌렀나 로봇 컨트롤 변수에 저장
	else if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) // esc 키
		glfwSetWindowShouldClose(window, true);
	else if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) { // 스페이스바
		SpaceBar = !SpaceBar;
		if (SpaceBar) { // 스페이스바가 눌린 상태로 바뀌었으면
			flag = true; // dist를 새로 계산하도록 신호를 줌.
		}
	}
}
