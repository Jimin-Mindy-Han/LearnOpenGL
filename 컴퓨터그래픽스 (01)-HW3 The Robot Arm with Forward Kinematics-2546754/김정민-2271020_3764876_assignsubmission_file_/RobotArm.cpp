...
int RobotControl = 0;
bool is_space_on = false; // 스페이스바 감지용 bool 변수
...
glm::mat4 new_objectXform = objectXform;
...
void myDisplay()
{
	...
	base_model = glm::translate(base_model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	base_model = glm::rotate(base_model, glm::radians(BaseSpin), glm::vec3(0.0f, 0.1f, 0.0f));
	DrawBase(base_model);

	glm::mat4 segment_model_1 = glm::translate(base_model, glm::vec3(0.0f, 0.4f, 0.0f));
	segment_model_1 = glm::rotate(segment_model_1, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 0.1f));
	DrawArmSegment(segment_model_1);

	glm::mat4 segment_model_2 = glm::translate(segment_model_1, glm::vec3(0.0f, 0.5f, 0.0f));
	segment_model_2 = glm::rotate(segment_model_2, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 0.1f));
	DrawArmSegment(segment_model_2);

	glm::mat4 wrist_model = glm::translate(segment_model_2, glm::vec3(0.0f, 0.5f, 0.0f));
	wrist_model = glm::rotate(wrist_model, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 0.1f));
	wrist_model = glm::rotate(wrist_model, glm::radians(WristTwistAng), glm::vec3(0.0f, 0.1f, 0.0f));
	DrawWrist(wrist_model);

	glm::mat4 finger_base_1 = glm::translate(wrist_model, glm::vec3(0.0f, 0.2f, 0.0f));
	finger_base_1 = glm::rotate(finger_base_1, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 0.4f));
	DrawFingerBase(finger_base_1);

	glm::mat4 finger_base_2 = glm::translate(wrist_model, glm::vec3(0.0f, 0.2f, 0.0f));
	finger_base_2 = glm::rotate(finger_base_2, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 0.4f));
	DrawFingerBase(finger_base_2);

	glm::mat4 finger_tip_1 = glm::translate(finger_base_1, glm::vec3(0.0f, 0.35f, 0.0f));
	finger_tip_1 = glm::rotate(finger_tip_1, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 0.4f));
	DrawFingerTip(finger_tip_1);

	glm::mat4 finger_tip_2 = glm::translate(finger_base_2, glm::vec3(0.0f, 0.35f, 0.0f));
	finger_tip_2 = glm::rotate(finger_tip_2, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 0.4f));
	DrawFingerTip(finger_tip_2);


	if (!is_space_on) {
		DrawObject(new_objectXform);
	}

	else if (is_space_on) {
		new_objectXform = wrist_model * objectXform;
		DrawObject(new_objectXform);
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) // 스페이스바 감지
		is_space_on = !is_space_on;
}
