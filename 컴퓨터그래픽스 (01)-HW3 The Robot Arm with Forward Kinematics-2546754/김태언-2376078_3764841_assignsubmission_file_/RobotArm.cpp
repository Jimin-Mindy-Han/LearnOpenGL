...
/// 로봇 팔 전체 x, z 평면 이동(Translate): 좌우, 앞뒤
float BaseTransX = -0.5f;  
float BaseTransZ = 0;
// 키보드 1
...
/// 관절 회전(z축 기준, 어깨: 앞뒤로 움직이는 관절임)
float ShoulderAng = -10;   
/// 관절 회전(z축 기준 회전, 팔꿈치: 앞뒤로 움직이는 관절임)
float ElbowAng = -120;
// 키보드 3
...
// EXTRA CREDIT //
bool isTeapotAttached = false; // false = 안 붙음, true = 붙음
...
// 수정 후 //
const char* ourObjectPath = "../../../resources/objects/teapot/teapot.obj";
...
void myDisplay()
{
	...
	glm::mat4 handModel = model;


	// 5. 손가락(Fingers)

	// Finger1
	glm::mat4 finger1Model = handModel;
	// 첫 번째 관절
	finger1Model = glm::translate(finger1Model, glm::vec3(0.0f, 0.2f, 0.0f));
	finger1Model = glm::rotate(finger1Model, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f));
	
	// 첫 번째 마디 그리기
	DrawFingerBase(finger1Model);

	// 두 번째 관절
	finger1Model = glm::translate(finger1Model, glm::vec3(0.0f, 0.35f, 0.0f));
	finger1Model = glm::rotate(finger1Model, glm::radians(FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f));
	
	// 손가락 끝 그리기
	DrawFingerTip(finger1Model);


	// Finger2
	glm::mat4 finger2Model = handModel;
	// 첫 번째 관절, 위쪽 집게랑은 대칭이어야 함(180도 회전)
	finger2Model = glm::translate(finger2Model, glm::vec3(0.0f, 0.2f, 0.0f));
	finger2Model = glm::rotate(finger2Model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	finger2Model = glm::rotate(finger2Model, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f));
	
	// 첫 번째 마디 그리기
	DrawFingerBase(finger2Model);
	
	// 두 번째 관절
	finger2Model = glm::translate(finger2Model, glm::vec3(0.0f, 0.35f, 0.0f));
	finger2Model = glm::rotate(finger2Model, glm::radians(FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f));
	
	// 손가락 끝 그리기
	DrawFingerTip(finger2Model);



	// EXTRA CREDIT 수정 후
	if (isTeapotAttached) //값이 TRUE일 때만.
	{
		glm::mat4 teapotLocalXform = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.4f, 0.0f));
		teapotLocalXform = glm::scale(teapotLocalXform, glm::vec3(0.08f, 0.08f, 0.08f));

		glm::mat4 attachedTeapotXform = handModel * teapotLocalXform;
...
void setupShader()
{
	...
	PhongShader = new Shader("../../../src/3.model_loading/2.robot_arm/model_loading.vs", "../../../src/3.model_loading/2.robot_arm/model_loading.fs");
	PhongShader->use();
	...
	FloorShader = new Shader("../../../src/3.model_loading/2.robot_arm/advanced_lighting.vs", "../../../src/3.model_loading/2.robot_arm/advanced_lighting.fs");
	FloorShader->use();
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// EXTRA CREDIT
	// 스페이스 바 누르면, 부울린변수 뒤집음.
	if (key == GLFW_KEY_SPACE && action == GLFW_PRESS)
		isTeapotAttached = !isTeapotAttached; // 값을 뒤집기
}
...
Plane::Plane()
{
	...
	floorTexture = loadTexture("../../../resources/objects/wood/wood.png");
