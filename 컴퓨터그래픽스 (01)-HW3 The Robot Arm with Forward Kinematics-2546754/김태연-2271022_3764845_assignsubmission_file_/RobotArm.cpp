...
glm::mat4 objectXform = glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f));
glm::mat4 objectXform1 = glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f));
glm::mat4 origin = glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f));
...
void myDisplay()
{
	...
	basemat = glm::translate(basemat, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	//objectXform = origin;
	if (RobotControl == 0 &&space) {
		objectXform = glm::translate(origin, glm::vec3( teapotTransX, 0.0f, +teapotTransZ)); //1번  티팟 위치
		//objectXform = basemat * objectXform;
	}if (RobotControl == 0 && !space) {
		teapotTransX = 0;
		teapotTransZ = 0;
		objectXform = objectXform1;
		origin = objectXform;	
	}
	//objectXform = glm::translate(basemat, glm::vec3(0.5f + teapotTransX, 0.0f, teapotTransZ));
	
	
	//objectXform = basemat * objectXform;
	float angle = glm::radians(teapotSpin); 
	glm::vec3 axis(0.0f, 1.0f, 0.0f);
	if (RobotControl == 1 && space) {
		
		
		glm::vec3 pivot = glm::vec3(basemat[3]); 
		glm::vec3 localAxis = glm::normalize(axis);

		
		glm::mat4 rotationMat = glm::translate(glm::mat4(1.0f), pivot) *
			glm::rotate(glm::mat4(1.0f), angle, localAxis) *
			glm::translate(glm::mat4(1.0f), -pivot);

		objectXform = rotationMat * origin;
	...
	basemat = glm::rotate(basemat, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(basemat);

	glm::mat4 armmat1 = glm::translate(basemat, glm::vec3(0.0f, 0.4f, 0.0f));
	//armmat1 = basemat * armmat1;
	glm::vec3 rotationAxis(0.0f, 0.0f, 1.0f);
	
	armmat1 = glm::rotate(armmat1, glm::radians(ShoulderAng), rotationAxis);
	DrawArmSegment(armmat1);

	glm::mat4 armmat2 = armmat1;
	armmat2 = glm::translate(armmat2, glm::vec3(0.0f, 0.5f, 0.0f));
	
	armmat2 = glm::rotate(armmat2, glm::radians(ElbowAng), rotationAxis);
	DrawArmSegment(armmat2);

	glm::mat4 whistmat = armmat2;
	whistmat = glm::translate(whistmat, glm::vec3(0.0f, 0.5f, 0.0f));
	if (RobotControl == 2 && space) {
		// basemat -> arm, wrist 행렬 적용
		glm::vec3 pivot = glm::vec3(whistmat[3]); // 회전 중심
		glm::vec3 worldAxis = glm::normalize(glm::vec3(armmat1 * glm::vec4(rotationAxis, 0.0f)));

		// pivot 기준 회전 적용
		glm::mat4 rotationMat = glm::translate(glm::mat4(1.0f), pivot) *
			glm::rotate(glm::mat4(1.0f), glm::radians(teapotEAng), worldAxis) *
			glm::translate(glm::mat4(1.0f), -pivot);
	...
	if (RobotControl == 3 && space) {
		glm::vec3 pivot = glm::vec3(whistmat[3]); 
		glm::vec3 worldAxis = glm::normalize(glm::vec3(whistmat * glm::vec4(rotationAxis, 0.0f)));
		glm::vec3 worldAxis1 = glm::normalize(glm::vec3(whistmat * glm::vec4(glm::vec3(0.0f, 1.0f, 0.0f), 0.0f)));

		
		glm::mat4 rotationMat = glm::translate(glm::mat4(1.0f), pivot) *
			glm::rotate(glm::mat4(1.0f), glm::radians(teapotWAng), worldAxis) *
			glm::rotate(glm::mat4(1.0f), glm::radians(teapotWTAng), worldAxis1) *
			glm::translate(glm::mat4(1.0f), -pivot);
	...
	whistmat = glm::rotate(whistmat, glm::radians(WristAng), rotationAxis);
	whistmat = glm::rotate(whistmat, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(whistmat);

	glm::mat4 FBmat1 = whistmat;
	FBmat1 = glm::translate(FBmat1, glm::vec3(0.0f, 0.2f, 0.0f));
	glm::mat4 FBmat2 = FBmat1;
	FBmat1 = glm::rotate(FBmat1, glm::radians(FingerAng1), rotationAxis);
	FBmat2 = glm::rotate(FBmat2, glm::radians(360 - FingerAng1), rotationAxis);
	DrawFingerBase(FBmat1);
	DrawFingerBase(FBmat2);


	glm::mat4 FTmat1 = FBmat1;
	FTmat1 = glm::translate(FTmat1, glm::vec3(0.0f, 0.35f, 0.0f));
	FTmat1 = glm::rotate(FTmat1, glm::radians(FingerAng2), rotationAxis);
	DrawFingerTip(FTmat1);

	glm::mat4 FTmat2 = FBmat2;
	FTmat2 = glm::translate(FTmat2, glm::vec3(0.0f, 0.35f, 0.0f));
	FTmat2 = glm::rotate(FTmat2, glm::radians(360 - FingerAng2), rotationAxis);
	DrawFingerTip(FTmat2);


	//origin = objectXform;
	objectXform1 = objectXform;
	
	objectXform = glm::scale(objectXform, glm::vec3(0.08f, 0.08f, 0.08f)); // 티팟 스케일 조정
	
}
...
int main()
{
	...
	//glfwSetKeyCallback(window, spacebar);
	// render loop
	// -----------
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)
	{
		...
		case 0: BaseTransX += xoffset; BaseTransZ -= yoffset;
			if (space) {
				teapotTransX += xoffset; teapotTransZ -= yoffset;
			}
			break;
		case 1: BaseSpin += xoffset * 180; 
			if (space) {
				teapotSpin += xoffset * 180;
			}
			break;
		case 2: ShoulderAng += yoffset * -90; ElbowAng += xoffset * 90;
			if (space) {
				teapotSAng += yoffset * -90; teapotEAng += xoffset * 90;
			}
			break;
		case 3: WristAng += yoffset * -180; WristTwistAng += xoffset * 180;
			if (space) {
				teapotWAng += yoffset * -180; teapotWTAng += xoffset * 180;
			}
			break;
