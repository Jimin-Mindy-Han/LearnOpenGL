// model_loading.cpp로 바꿔서 LearnOpenGL파일에서 실행하였습니다. (3.model_loading)
...
void myDisplay()
{
	...
	// ADD YOUR ROBOT RENDERING STUFF HERE     ////////////////////////////밑에부터 코드수정부분/////////////////////////
	
// 내부 함수에 있는 숫자 잘 확인해서 가져다쓰기
	//이동 >> 회전 순서 지키기 !


	// 원점에서 시작해서 마우스이동, 회전 1번
	...
	// 어깨: 키보드 2번
	model = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f)); // 베이스 부품 높이(0.4)만큼 위로 이동
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f)); // 어깨 굽히기
	DrawArmSegment(model);

	//Elbow: 키보드 3번
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f)); // 팔 부품 길이(0.5)만큼 위로 이동
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f)); // 팔꿈치 굽히기
	DrawArmSegment(model); 

	// wrist: 키보드 4번
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f)); // 팔 부품 길이(0.5)만큼 위로 이동
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f)); // 손목 꺾기
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f)); // 손목 비틀기
	DrawWrist(model);

	// 손가락들: 키보드 5번
	model = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f)); // 손목 부품 길이(0.2)만큼 이동
	glm::mat4 finger = model; // 시작점 세팅 lfm,rfm,mfm에서 받아올 아이임 

	// 첫 세팅 기준 왼쪽에 있는 애 (왼쪽으로 0.05)
	glm::mat4 lfm = finger; // left finger mat길어서 lfm(finger에서 시작점 받아오기)
	lfm = glm::translate(lfm, glm::vec3(-0.05f, 0.0f, 0.0f)); // 왼쪽으로 0.05, 숫자는 고정치(받아온거) 아니고 조정가능한데, 커지면 틈이 생기고 좁히면 너무 좁아짐
	lfm = glm::rotate(lfm, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f)); // X축 기준
	DrawFingerBase(lfm);
	lfm = glm::translate(lfm, glm::vec3(0.0f, 0.35f, 0.0f)); 
	lfm = glm::rotate(lfm, glm::radians(FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f)); 
	DrawFingerTip(lfm);

	
	
	// 가운데 (방향 설정 중요!! 앵글 양 사이드에 있는 애들이랑 반대로 작동)
	glm::mat4 mfm = finger; // mid finger mat길어서 mfm(finger에서 시작점 받아오기)
	mfm = glm::rotate(mfm, glm::radians(-FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f)); // 가운데는 반대로 움직여야해서 -붙여서(방향바꾸기)
	DrawFingerBase(mfm);
	mfm = glm::translate(mfm, glm::vec3(0.0f, 0.35f, 0.0f));
	mfm = glm::rotate(mfm, glm::radians(-FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f));
	DrawFingerTip(mfm);


	// 첫 세팅 기준, 오른쪽애 있는 애 (오른쪽으로 0.05)
	glm::mat4 rfm = finger; // right finger mat길어서 rfm(finger에서 시작점 받아오기)
	rfm = glm::translate(rfm, glm::vec3(0.05f, 0.0f, 0.0f)); // 오른쪽으로 0.05띄우기 1로 띄우면 얘만 동떨어져 있게됨
	rfm = glm::rotate(rfm, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f)); // X축 기준 마찬가지로
	DrawFingerBase(rfm);
	rfm = glm::translate(rfm, glm::vec3(0.0f, 0.35f, 0.0f));
	rfm = glm::rotate(rfm, glm::radians(FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f));
	DrawFingerTip(rfm);
...
void setupShader()
{
	PhongShader = new Shader("1.model_loading.vs", "1.model_loading.fs");
	PhongShader->use();

	// Light attributes
	PhongShader->setVec3("lightColor", lightColor);

	FloorShader = new Shader("1.advanced_lighting.vs", "1.advanced_lighting.fs");
	FloorShader->use();
