...
bool TeapotFollowWrist = true; // 티팟이 따라갈지에 대한 스페이스바 토글
...
void myDisplay()
{
	...
	// shuolder
	glm::mat4 shoulder = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f));
	...
	// first finger1
	glm::mat4 finger1 = glm::translate(wrist, glm::vec3(0.05f, 0.15f, 0.0f));
	finger1 = glm::rotate(finger1, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(finger1);

	glm::mat4 fingertip1 = glm::translate(finger1, glm::vec3(0.0f, 0.3f, 0.0f));
	fingertip1 = glm::rotate(fingertip1, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(fingertip1);

	// second finger2
	glm::mat4 finger2 = glm::translate(wrist, glm::vec3(-0.05f, 0.15f, 0.0f));
	finger2 = glm::rotate(finger2, glm::radians(-FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(finger2);

	glm::mat4 fingertip2 = glm::translate(finger2, glm::vec3(0.0f, 0.3f, 0.0f));
	fingertip2 = glm::rotate(fingertip2, glm::radians(-FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(fingertip2);

	// ---------------------------------------------------------------------------------------------

	if (TeapotFollowWrist) // 핸드의 손목에 붙어 티팟이 움직이도록 하는 것
	{
		glm::mat4 teapot = wrist * glm::translate(glm::mat4(1.0f), glm::vec3(0.2f, 0.2f, 0.0f));
		teapot = teapot * glm::scale(glm::mat4(1.0f), glm::vec3(0.08f));
		DrawObject(teapot);
	}

	else // 원래 자리에 그대로 두는 것
	{
		DrawObject(objectXform);
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
		...
		TeapotFollowWrist = !TeapotFollowWrist; // 스페이스 바를 누를 때마다 토글(반전)되도록 하는 기능
}
