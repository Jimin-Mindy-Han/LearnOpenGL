...
void myDisplay()
{
	...
	//4번입력
	//Wrist
	//WristAng(마우스y축:회전(z축))
	//WrsitTwistAng(마우스x축:회전(y축))
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	...
	//Finger1(마우스x축:base회전(z축)/마우스t축:tip회전(z축))
	glm::mat4 finger1 = glm::rotate(model, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(finger1);
	finger1 = glm::translate(finger1, glm::vec3(0.0f, 0.35f, 0.0f));
	finger1 = glm::rotate(finger1, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(finger1);
	//Finger2(손가락1의반대로회전)
	glm::mat4 finger2 = glm::rotate(model, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(finger2);
	finger2 = glm::translate(finger2, glm::vec3(0.0f, 0.35f, 0.0f));
