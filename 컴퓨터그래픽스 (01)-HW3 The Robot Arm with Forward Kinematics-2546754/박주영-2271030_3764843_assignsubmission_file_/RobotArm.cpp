/*
HW3 Robot Arm 2271030 Park Juyoung
Instructions : 
	-Controls : press number key 1 to 5 to select joint, do mouse drag to rotate
	-(extra) press Space key to toggle teapot follow mode
	-for basic and extra credit, there are some changes 
			for processInput() and myDisplay(), and new variable 'followWrist'
*/
#include <glad/glad.h>
...
// CALLBACKS	mouse_callback, processInput -> 로봇 팔 관절 각도 바꿈
...
//for extra credit
bool followWrist = true;	//teapot model이 wrist를 follow하는지의 여부

void myDisplay() // Hierarchical modeling해서 로봇 팔 조립
{
	...
	// ground -> base -> shoulder -> arm -> eblow -> arm -> wrist -> 
	// fingers(DrawFingerBase _ DrawFingerTip)
	// 매 단계에서 translate, rotate, scale 적용 시 현재까지의 변환을 누적(product)
	// glm::mat4 : 각 단계의 변환 표현
	// DrawJoint 함수 : 관절 표현 - 연결부에 호출 / 이동된 위치, but not rotate

	//Base (spin/BaseTransX/BaseTransZ)
	 ...
	 DrawJoint(model);	//Base와 Shoulder 사이 관절(model in 전역좌표계)

	//Shoulder	//base 기준은 이동, but 전역에서는 제자리에서 회전 -> translate 안 함
	 glm::mat4 shoulder = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0.0f,0.0f,1.0f));
	 DrawArmSegment(shoulder);

	//Arm

	//Eblow
	 glm::mat4 elbow = glm::translate(shoulder, glm::vec3(0.0f, 0.7f, 0.0f)); //이동
	 DrawJoint(elbow);	//Shoulder와 Eblow 사이 관절(eblow=shoulder의 자식)
	 elbow = glm::rotate(elbow, glm::radians(ElbowAng), glm::vec3(0.0f,0.0f,1.0f));	//회전
	 DrawArmSegment(elbow);

	//Arm

	//Wrist
	 glm::mat4 wrist = glm::translate(elbow, glm::vec3(0.0f, 0.7f, 0.0f));
	 DrawJoint(wrist); //Eblow와 Wrist 사이 관절
	...
	//Fingers
	 glm::mat4 finger1 = glm::translate(wrist, glm::vec3(0.05f, 0.2f, 0.0f));
	 DrawJoint(finger1);
	 ...
	 glm::mat4 finger2 = glm::translate(wrist, glm::vec3(-0.05f, 0.2f, 0.0f));
	 DrawJoint(finger2);
	 finger2 = glm::rotate(finger2, glm::radians(FingerAng2), glm::vec3(0.0f,0.0f,1.0f));
	...
	///////teapot Model/////
	 glm::mat4 teapotMD;
	 if(followWrist){	// followWrist가 true 면 wrist 좌표계 기준으로 teapot 모델을 그림
		teapotMD = wrist;
		teapotMD = glm::translate(teapotMD, glm::vec3(0.0f, handLength, 0.0f));
		teapotMD = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(0,0,1));
	 }
	 else{
		teapotMD = glm::mat4(1.0f);	//전역 좌표에서 그림
		tepotMD = glm::translate(teapotMD, glm::vec3(1.0f, 0.0f, 0.0f));
	 }
	 teapotMD *= objectXform;	//크기, 중심 보정
	DrawObject(teapotMD);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	//for extra credit
		...
		followWrist =! followWrist;	//space bar 누르면 t/f 변환
	}
}
