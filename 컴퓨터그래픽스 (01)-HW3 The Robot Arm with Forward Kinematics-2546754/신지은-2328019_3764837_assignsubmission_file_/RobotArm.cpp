...
//주전자가 wrist따라 움직일지 안 움직일 지 boolean 변수
bool teapotmovable=false;
...
void DrawFingerTip(glm::mat4 model); //적절한 순서에 따라 call, 적당한 위치에 배치되려면 modeling trasformation을 채워서 보내야함.
...
void myDisplay()
{
	...
	DrawGroundPlane(model); //바닥그리는 함수
	

	// ADD YOUR ROBOT RENDERING STUFF HERE     /////////////////////////////////////////////////////
	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	model = glm::rotate(model, BaseSpin/80.0f, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(model);
	
	model = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f));
	model = glm::rotate(model, ShoulderAng / 80.0f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model);

	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, ElbowAng / 80.0f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model);

	model= glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, WristAng / 80.0f, glm::vec3(0.0f, 0.0f, 1.0f));
	model = glm::rotate(model, WristTwistAng / 80.0f, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(model);

	glm::mat4 wristmodel = model;
	model = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f));
	model = glm::rotate(model, glm::radians(30.0f), glm::vec3(1.0f, 0.0f, 0.0f)); //y축으로 회전시키면 자기 머리 위 방향이므로 원하는 대로 display 안됨. x축방향 또는 z축방향으로 회전 시켜야함.
	model = glm::rotate(model, FingerAng1 / 80.0f, glm::vec3(1.0f, 0.0f, 0.0f));
	DrawFingerBase(model);

	model = glm::translate(model, glm::vec3(0.0f, 0.35f, 0.0f));
	model = glm::rotate(model, FingerAng2 / 80.0f, glm::vec3(1.0f, 0.0f, 0.0f));
	DrawFingerTip(model);
	...
	if (teapotmovable) {
		...
		model = glm::rotate(model, WristAng / 80.0f, glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::rotate(model, WristTwistAng / 80.0f, glm::vec3(0.0f, 1.0f, 0.0f));
		DrawObject(objectXform*model);
	}
	else DrawObject(objectXform); //주전자그리는 함수 
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
		RobotControl = key - GLFW_KEY_1; //RobotControl 변수에 어떤 키를 눌렀는지 저장이 됨. //숫자 1 누르면 1-1=0값이 RobotControl 안에 들어가고, 숫자 2 누르면 2-1=1값이 RobotControl 안에 들어가고...
	else if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) //space바 누르면 teapotmovable 변경되도록!
		teapotmovable = !teapotmovable;
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown) //키를 누르면서 마우스 왼쪽버튼을 움직이면 아래 변수들이 변화함.
	{
		switch (RobotControl)
		{
		...
		} //즉, modeling trasformation할 때 위에 변수들을 이용해야함.
	} 
...
void DrawBase(glm::mat4 model) //Draw 함수들
{
...
void DrawArmSegment(glm::mat4 model)
{
	...
	Base = glm::scale(Base, glm::vec3(0.1f, 0.5f, 0.1f)); //y축 길이 0.5f
	glm::mat4 InBase = glm::inverse(Base);
