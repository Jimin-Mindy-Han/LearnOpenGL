...
void myDisplay()
{
	...
	B = glm::translate(B, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	B = glm::rotate(B, glm::radians(BaseSpin), glm::vec3(0, 1, 0));
	DrawBase(B);

	// ¾î±ú
	glm::mat4 S = B;
	S = glm::translate(S, glm::vec3(0.0f, h1, 0.0f));
	S = glm::rotate(S, glm::radians(ShoulderAng), glm::vec3(0, 0, 1));
	DrawArmSegment(S);

	// ÆÈ²ÞÄ¡
	glm::mat4 E = S;
	E = glm::translate(E, glm::vec3(0.0f, h2, 0.0f));
	E = glm::rotate(E, glm::radians(ElbowAng), glm::vec3(0, 0, 1));
	DrawArmSegment(E);

	// ¼Õ¸ñ
	glm::mat4 W = E;
	W = glm::translate(W, glm::vec3(0.0f, h3, 0.0f));
	W = glm::translate(W, glm::vec3(0.0f, h4, 0.0f));
	W = glm::rotate(W, glm::radians(WristAng), glm::vec3(0, 0, 1));
	W = glm::rotate(W, glm::radians(WristTwistAng), glm::vec3(0, 1, 0));
	DrawWrist(W);

	// ¿À¸¥ÂÊ ¼Õ°¡¶ô
	glm::mat4 FR = W;
	FR = glm::translate(FR, glm::vec3(fGap, fStart, 0.0f));
	FR = glm::rotate(FR, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(FR);

	glm::mat4 FRT = FR;
	FRT = glm::translate(FRT, glm::vec3(0.0f, fLen, 0.0f));
	FRT = glm::rotate(FRT, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(FRT);

	// ¿ÞÂÊ ¼Õ°¡¶ô
	glm::mat4 FL = W;
	FL = glm::translate(FL, glm::vec3(-fGap, fStart, 0.0f));
	FL = glm::rotate(FL, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(FL);

	glm::mat4 FLT = FL;
	FLT = glm::translate(FLT, glm::vec3(0.0f, fLen, 0.0f));
	FLT = glm::rotate(FLT, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(FLT);
