...
//teapot follow toggle
bool g_TeapotFollowWrist = false;
...
void myDisplay()
{
	...
	//1,2번 동작: XZ이동 + y축 스핀
	glm::mat4 M = glm::mat4(1.0f); //초기상태
	M = glm::translate(M, glm::vec3(BaseTransX, 0.0f, BaseTransZ)); //로봇 기준점 XZ평면에서 이동
	M = glm::rotate(M, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f)); //월드의 y축 기준으로 BaseSpin 회전

	DrawBase(M); //base 그리기

	//shoulder 기준좌표
	glm::mat4 M_shoulder = M; //베이스 위에 조인트 올리기
	M_shoulder = glm::translate(M_shoulder, glm::vec3(0.0f, 0.40f, 0.0f));

	//3번 동작: shoulder (mcs)z축 회전
	M_shoulder = glm::rotate(M_shoulder, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(M_shoulder); //shoulder 그리기

	//elbow 기준 좌표
	glm::mat4 M_elbow = M_shoulder;
	M_elbow = glm::translate(M_elbow, glm::vec3(0.0f, 0.50f, 0.0f));
	
	// elbow (mcs)z축 회전
	M_elbow = glm::rotate(M_elbow, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(M_elbow); //elbow 그리기

	//wrist 기준 좌표
	glm::mat4 M_wrist = M_elbow;
	M_wrist = glm::translate(M_wrist, glm::vec3(0.0f, 0.50f, 0.0f));

	//4번 동작: wrist (mcs)z축 회전 
	M_wrist = glm::rotate(M_wrist, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	M_wrist = glm::rotate(M_wrist, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawWrist(M_wrist);

	//5번 동작: 집게동작 구현

	//left
	glm::mat4 M_f1 = M_wrist;
	M_f1 = glm::translate(M_f1, glm::vec3(0.0f, 0.20f, 0.0f));
	M_f1 = glm::rotate(M_f1, glm::radians(+FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(M_f1);
	
	glm::mat4 M_f1_tip = glm::translate(M_f1, glm::vec3(0.0f, 0.35f, 0.0f));
	M_f1_tip = glm::rotate(M_f1_tip, glm::radians(+FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(M_f1_tip);

	//right
	glm::mat4 M_f2 = M_wrist;
	M_f2 = glm::translate(M_f2, glm::vec3(0.0f, 0.20f, 0.0f));
	M_f2 = glm::rotate(M_f2, glm::radians(-FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(M_f2);
	
	glm::mat4 M_f2_tip = glm::translate(M_f2, glm::vec3(0.0f, 0.35f, 0.0f));
	M_f2_tip = glm::rotate(M_f2_tip, glm::radians(-FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(M_f2_tip);


	//teapot toggle
	glm::mat4 teapotLocal = glm::mat4(1.0f);
	teapotLocal = glm::translate(teapotLocal, glm::vec3(0.0f, 0.18f, 0.0f)); //wrist 약간 위에 올리기
	teapotLocal = glm::rotate(teapotLocal, glm::radians(-90.0f), glm::vec3(0, 0, 1));
	teapotLocal = glm::scale(teapotLocal, glm::vec3(0.08f));

	if (g_TeapotFollowWrist)
		DrawObject(M_wrist * teapotLocal);
	else
		DrawObject(objectXform);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	//space key toggle
		...
		g_TeapotFollowWrist = !g_TeapotFollowWrist;
	}
}
