...
void myDisplay()
{
	...
	static float s_baseMoveX, s_baseMoveZ;
	static float s_baseYaw;
	static float s_shoulder;
	static float s_elbow;
	static float s_wristPitch;
	static float s_wristRoll;
	static float s_finger1, s_finger2;

	if (!s_inited) {
		s_baseMoveX = BaseTransX;
		s_baseMoveZ = BaseTransZ;
		s_baseYaw = BaseSpin;
		s_shoulder = ShoulderAng;
		s_elbow = ElbowAng;
		s_wristPitch = WristAng;
		s_wristRoll = WristTwistAng;
		s_finger1 = FingerAng1;
		s_finger2 = FingerAng2;
		s_inited = true;
	}

	// 1: BaseYaw, 2: Shoulder, 3: Elbow, 4: WristPitch, 5: WristRoll
	switch (RobotControl) {
	case 0: 
		s_baseYaw = BaseSpin;
		break;
	...
	// 1번 Base
	m = glm::translate(m, glm::vec3(s_baseMoveX, 0.0f, s_baseMoveZ));// 로봇 전체 좌우, 앞뒤 이동
	m = glm::rotate(m, glm::radians(s_baseYaw), glm::vec3(0.0f, 1.0f, 0.0f));// Y축 기준으로 회전
	DrawBase(m);

	// 2번 Shoulder
	m = glm::translate(m, glm::vec3(0.0f, 0.4f, 0.0f)); // 2번째 조인트로 이동
	m = glm::rotate(m, glm::radians(s_shoulder), glm::vec3(1.0f, 0.0f, 0.0f));// X축 기준으로 회전
	DrawArmSegment(m); 

	// 3번 Elbow
	m = glm::translate(m, glm::vec3(0.0f, 0.5f, 0.0f));// 3번째 조인트로 이동
	m = glm::rotate(m, glm::radians(s_elbow), glm::vec3(1.0f, 0.0f, 0.0f));// X축 기준으로 회전
	DrawArmSegment(m);

	// 4번 Wrist 
	m = glm::translate(m, glm::vec3(0.0f, 0.5f, 0.0f)); // 4번 조인트로 이동
	m = glm::rotate(m, glm::radians(s_wristPitch), glm::vec3(1.0f, 0.0f, 0.0f));// X축 기준으로 회전
	m = glm::rotate(m, glm::radians(s_wristRoll), glm::vec3(0.0f, 1.0f, 0.0f));// Y축 기준으로 회전
	DrawWrist(m);

	// 5번Finger
		
	{	// Right
		glm::mat4 mr = glm::translate(m, glm::vec3(-0.05f, 0.2f, 0.0f)); // 5번 조인트로 위치 이동
		mr = glm::rotate(mr, glm::radians(s_finger2), glm::vec3(0.0f, 0.0f, 1.0f)); //Z축 기준으로 회전
		DrawFingerBase(mr);
		DrawFingerTip(glm::translate(mr, glm::vec3(0.0f, 0.3f, 0.0f)));
		
		
		// Left
		glm::mat4 ml = glm::translate(m, glm::vec3(+0.05f, 0.2f, 0.0f)); // 5번 조인트로 위치 이동
		ml = glm::rotate(ml, glm::radians(s_finger1), glm::vec3(0.0f, 0.0f, 1.0f));// Z축 기준으로 회전
		DrawFingerBase(ml);
		DrawFingerTip(glm::translate(ml, glm::vec3(0.0f, 0.3f, 0.0f)));
