...
int RobotControl = 0;
bool IsObjectSynchronized = false;
bool WasObjectSynchronized = false;    
glm::mat4 ObjectLocalOffset = glm::mat4(1.0f);
...
void myDisplay()
{
	...
	// 1 : Base 마우스 움직임에 따라 translate 하는 것 반영
	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	// 2 : 마우스 움직임에 따라 전체 Rotate 하는 것 반영
	model = glm::rotate(model, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));

	// Base 그리기
	DrawBase(model);

	// 3 : 마우스 움직임에 따라 Arm Rotate 하는 것 반영

	glm::mat4 modelS = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f));
	modelS = glm::rotate(modelS, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(modelS);	// Shoulder

	glm::mat4 modelE = glm::translate(modelS, glm::vec3(0.0f, 0.5f, 0.0f));
	modelE = glm::rotate(modelE, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(modelE);	// Elbow

	// 4 : 마우스 움직임에 따라 Wrist Rotate 하는 것 반영
	// 4-1 : WristAng 반영
	glm::mat4 modelW = glm::translate(modelE, glm::vec3(0.0f, 0.5f, 0.0f));
	modelW = glm::rotate(modelW, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));

	// 4-2 : WristTwistAng 반영
	modelW = glm::rotate(modelW, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(modelW);

	// 5 : 마우스 움직임에 따라 Finger Rotate 하는 것 반영
	// 5-1 : 왼쪽 Finger (FL)
	glm::mat4 modelFL = glm::translate(modelW, glm::vec3(0.0f, 0.2f, 0.0f));
	modelFL = glm::rotate(modelFL, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(modelFL);
	modelFL = glm::translate(modelFL, glm::vec3(0.0f, 0.35f, 0.0f));
	modelFL = glm::rotate(modelFL, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(modelFL);

	// 5-2 : 오른쪽 Finger (FR)
	glm::mat4 modelFR = glm::translate(modelW, glm::vec3(0.0f, 0.2f, 0.0f));
	modelFR = glm::rotate(modelFR, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(modelFR);
	modelFR = glm::translate(modelFR, glm::vec3(0.0f, 0.35f, 0.0f));
	modelFR = glm::rotate(modelFR, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(modelFR);
	
	// Extra Credit : 스페이스 누르면 오브젝트 Wrist와 동기화시키기
	glm::mat4 modelO = objectXform;

	if (IsObjectSynchronized) {
		if (!WasObjectSynchronized) {
			ObjectLocalOffset = glm::inverse(modelW) * objectXform;
			WasObjectSynchronized = true;
		}
		modelO = modelW * ObjectLocalOffset;
	}
	else {
		WasObjectSynchronized = false;
	}
	DrawObject(modelO);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
		...
		IsObjectSynchronized = !IsObjectSynchronized;
}
