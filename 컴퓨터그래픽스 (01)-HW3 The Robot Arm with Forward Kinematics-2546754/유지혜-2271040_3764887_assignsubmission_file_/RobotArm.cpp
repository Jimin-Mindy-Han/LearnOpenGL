/*
This code assumes the following file structure at runtime:

  (Executable Directory)
  戍式式 RobotArm.cpp
  戍式式 model_loading.vs
  戍式式 model_loading.fs
  戍式式 advanced_lighting.vs
  戍式式 advanced_lighting.fs
  戍式式 teapot.obj
  戍式式 wood.png
  戌式式 assimp-vc140-mt.dll

- If shader or texture loading fails, please make sure that:
	All *.vs, *.fs, *.obj, and *.png files are located in the same folder.
...
void myDisplay()
{
	...
	modelBase = glm::translate(modelBase, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	modelBase = glm::rotate(modelBase, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(modelBase);

	// 2. Shoulder
	glm::mat4 modelShoulder = glm::translate(modelBase, glm::vec3(0.0f, 0.4f, 0.0f));
	modelShoulder = glm::rotate(modelShoulder, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(modelShoulder);

	// 3. Elbow
	glm::mat4 modelElbow = glm::translate(modelShoulder, glm::vec3(0.0f, 0.5f, 0.0f));
	modelElbow = glm::rotate(modelElbow, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(modelElbow);

	// 4. Wrist
	glm::mat4 modelWrist = glm::translate(modelElbow, glm::vec3(0.0f, 0.5f, 0.0f));
	modelWrist = glm::rotate(modelWrist, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	modelWrist = glm::rotate(modelWrist, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(modelWrist);

	// 5. Fingers 
	glm::mat4 modelFinger1 = glm::translate(modelWrist, glm::vec3(0.05f, 0.1f, 0.0f));
	modelFinger1 = glm::rotate(modelFinger1, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(modelFinger1);

	glm::mat4 modelFingerTip1 = glm::translate(modelFinger1, glm::vec3(0.0f, 0.35f, 0.0f));
	modelFingerTip1 = glm::rotate(modelFingerTip1, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f)); 
	DrawFingerTip(modelFingerTip1);

	glm::mat4 modelFinger2 = glm::translate(modelWrist, glm::vec3(0.05f, 0.1f, 0.0f));
	modelFinger2 = glm::rotate(modelFinger2, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(modelFinger2);

	glm::mat4 modelFingerTip2 = glm::translate(modelFinger2, glm::vec3(0.0f, 0.35f, 0.0f));
	modelFingerTip2 = glm::rotate(modelFingerTip2, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f)); 
	DrawFingerTip(modelFingerTip2);
