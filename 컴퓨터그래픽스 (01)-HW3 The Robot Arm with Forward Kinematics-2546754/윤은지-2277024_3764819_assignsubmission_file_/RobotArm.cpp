...
// EXTRA CREDIT: Teapot attachment toggle
bool isTeapotAttached = false; // false = 안 붙음, true = 붙음 (초기 상태는 안 붙은 상태)
...
void myDisplay()
{
	...
	//DrawObject(objectXform); 주석 처리 안하면 주전자가 Extra credit으로 인해 주전자가 두 번 보이는 문제 발생

	// ADD YOUR ROBOT RENDERING STUFF HERE     /////////////////////////////////////////////////////
	// 베이스
	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ)); // X, Z 평면에서 베이스 이동
	model = glm::rotate(model, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f)); // Y축 기준으로 베이스 회전
	DrawBase(model);

	// 어깨 관절
	model = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f)); // 베이스 상단 관절 위치로 이동
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(1.0f, 0.0f, 0.0f)); // X축 기준으로 2번 joint 회전
	DrawArmSegment(model);

	// 팔꿈치 관절
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f)); // 아래팔 끝 관절 위치로 이동
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(1.0f, 0.0f, 0.0f)); // X축 기준으로 3번 joint 회전
	DrawArmSegment(model);

	// 손목 관절
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f)); // 위팔 끝 관절 위치로 이동
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f)); // Y축 기준으로 4번 joint 비틀기
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(1.0f, 0.0f, 0.0f)); // X축 기준으로 4번 joint 굽히기
	DrawWrist(model);

	// 손가락 관절
	glm::mat4 wristModel = model; // 현재 손목까지의 변환 상태를 저장

	// 첫 번째 손가락
	{
		glm::mat4 fingerModel = wristModel; // 저장된 4번 상태에서 시작 (집게 두 개라서 저장하고 복원)
		fingerModel = glm::translate(fingerModel, glm::vec3(0.0f, 0.2f, 0.0f)); // 손가락 시작 위치로 이동
		fingerModel = glm::rotate(fingerModel, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f)); // 손가락 굽히기
		DrawFingerBase(fingerModel);

		fingerModel = glm::translate(fingerModel, glm::vec3(0.0f, 0.35f, 0.0f)); // 손가락 끝 위치로 이동
		fingerModel = glm::rotate(fingerModel, glm::radians(FingerAng2), glm::vec3(1.0f, 0.0f, 0.0f)); // 손가락 끝 굽히기
		DrawFingerTip(fingerModel);
	}

	// 두 번째 손가락
	{
		glm::mat4 fingerModel = wristModel; // 다시 저장된 손목 상태에서 시작
		fingerModel = glm::rotate(fingerModel, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // 180도 돌려서 반대편을 그리게 함
		fingerModel = glm::translate(fingerModel, glm::vec3(0.0f, 0.2f, 0.0f)); // 손가락 시작 위치로 이동
		fingerModel = glm::rotate(fingerModel, glm::radians(FingerAng1), glm::vec3(1.0f, 0.0f, 0.0f)); // 손가락 굽히기
		DrawFingerBase(fingerModel);
	...
	// EXTRA CREDIT: Check if the teapot should be attached to the wrist
	if (isTeapotAttached)
	{
		// 상태 1: 주전자가 집게에 붙어있을 때
		glm::mat4 attachedTeapotXform = wristModel; // 손목의 좌표계에서 시작
		attachedTeapotXform = glm::translate(attachedTeapotXform, glm::vec3(0.0f, 0.45f, 0.0f)); // 손목 관절에서 살짝 위로 이동
		attachedTeapotXform = glm::scale(attachedTeapotXform, glm::vec3(0.08f, 0.08f, 0.08f));     // 크기 조절

		DrawObject(attachedTeapotXform);
	}
	else
	{
		// 주전자가 손목에 붙어 있지 않을 때는 원래의 위치에 고정
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		// EXTRA CREDIT: Space bar toggles teapot attachment
		isTeapotAttached = !isTeapotAttached; // bool 변수의 값을 반대로 뒤집음 (false -> true, true -> false)
	}
}
