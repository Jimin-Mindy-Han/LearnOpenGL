...
int RobotControl = 0;
bool isSpaceBarPressed = false; //스페이스바 토글 용 
...
glm::mat4 lastFrameWristModel = glm::mat4(1.0f); //손목의 위치를 담을 변수
...
void myDisplay()
{
	...
	//Arm 그리기
	//Arm을 그릴 땐, Base의 끝점까지 translate를 시켜야
	...
	//스페이스가 눌린 상태라면 주전자가 손목의 움직임을 따라감
	if (isSpaceBarPressed) {
		glm::mat4 howWristMoved = model * glm::inverse(lastFrameWristModel);
		//전 프레임의 손목 위치 <-> 현재 프레임의 손목 위치 차이 : 한 프레임동안 움직인 정도를 역행렬을 곱해 구함. 그리고 그 움직임을 howWristMoved에 저장.
		objectXform = howWristMoved * objectXform;
		//주전자를 손목이 움직인 만큼 움직여줌
	}
	DrawObject(objectXform);
	lastFrameWristModel = model; //다음 프레임의 손목 위치 기준점이 됨.

	//FingerBase 1 (위에거) 그리기
