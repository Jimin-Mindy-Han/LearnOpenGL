...
void myDisplay()
{
	...
	baseModel = glm::rotate(baseModel, glm::radians(BaseSpin), glm::vec3(0, 1, 0));
	DrawBase(baseModel);

	glm::mat4 shoulderModel = glm::translate(baseModel, glm::vec3(0.0f, 0.4f, 0.0f));
	shoulderModel = glm::rotate(shoulderModel, glm::radians(ShoulderAng), glm::vec3(0, 0, 1));
	DrawArmSegment(shoulderModel);

	glm::mat4 elbowModel = glm::translate(shoulderModel, glm::vec3(0.0f, 0.5f, 0.0f));
	elbowModel = glm::rotate(elbowModel, glm::radians(ElbowAng), glm::vec3(0, 0, 1));
	DrawArmSegment(elbowModel);

	glm::mat4 wristModel = glm::translate(elbowModel, glm::vec3(0.0f, 0.5f, 0.0f));
	wristModel = glm::rotate(wristModel, glm::radians(WristAng), glm::vec3(0, 0, 1));
	wristModel = glm::rotate(wristModel, glm::radians(WristTwistAng), glm::vec3(0, 1, 0));
	DrawWrist(wristModel);

	const float wristBallY = 0.20f;
	const float sphereR = 0.06f;
	const float baseCenter = 0.20f;
	const float baseHalf = 0.15f;
	const float attachOffset = sphereR - (baseCenter - baseHalf);
	const float extraSpread = -30.0f;
	float mid = 0.5f * (FingerAng1 + FingerAng2);
	float diff = (FingerAng1 > FingerAng2) ? (FingerAng1 - FingerAng2) : (FingerAng2 - FingerAng1);
	float dir = (FingerAng1 >= FingerAng2) ? 1.0f : -1.0f;
	float halfGap = 0.5f * diff + extraSpread;
	float leftAngle = mid + dir * halfGap;
	float rightAngle = mid - dir * halfGap;

	glm::mat4 wristBall = wristModel;
	wristBall = glm::translate(wristBall, glm::vec3(0.0f, wristBallY, 0.0f));

	glm::mat4 leftFinger = wristBall;
	leftFinger = glm::rotate(leftFinger, glm::radians(-90.0f), glm::vec3(0, 0, 1));
	leftFinger = glm::rotate(leftFinger, glm::radians(leftAngle), glm::vec3(0, 0, 1));
	leftFinger = glm::translate(leftFinger, glm::vec3(attachOffset, 0.0f, 0.0f));
	DrawFingerBase(leftFinger);

	glm::mat4 leftTip = glm::translate(leftFinger, glm::vec3(0.0f, 0.35f, 0.0f));
	leftTip = glm::rotate(leftTip, glm::radians(90.0f), glm::vec3(0, 0, 1));
	DrawFingerTip(leftTip);

	glm::mat4 rightFinger = wristBall;
	rightFinger = glm::rotate(rightFinger, glm::radians(90.0f), glm::vec3(0, 0, 1));
	rightFinger = glm::rotate(rightFinger, glm::radians(rightAngle), glm::vec3(0, 0, 1));
	rightFinger = glm::translate(rightFinger, glm::vec3(attachOffset, 0.0f, 0.0f));
	DrawFingerBase(rightFinger);

	glm::mat4 rightTip = glm::translate(rightFinger, glm::vec3(0.0f, 0.35f, 0.0f));
	rightTip = glm::rotate(rightTip, glm::radians(-90.0f), glm::vec3(0, 0, 1));
	DrawFingerTip(rightTip);


	static bool followWrist = false;
	static bool prevSpace = false;
	static glm::mat4 wristToObj = glm::mat4(1.0f);

	int spaceState = glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_SPACE);
	bool spaceDown = (spaceState == GLFW_PRESS);

	if (spaceDown && !prevSpace) {
		if (!followWrist) {
			wristToObj = glm::inverse(wristModel) * objectXform;
			followWrist = true;
		}
		else {
			objectXform = wristModel * wristToObj;
			followWrist = false;
		}
	}
	prevSpace = spaceDown;

	glm::mat4 objModel = followWrist ? (wristModel * wristToObj) : objectXform;
	DrawObject(objModel);
