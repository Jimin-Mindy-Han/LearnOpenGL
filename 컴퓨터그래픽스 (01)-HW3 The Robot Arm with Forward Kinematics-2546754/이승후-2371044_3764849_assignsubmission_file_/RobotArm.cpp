...
void myDisplay()
{
	...
	static float jointAngles[5] = { 0,0,0,0,0 };				// Joint rotation angles
	static int selectedJoint = -1;  							// Selected joint for motion control (0-4, default -1)
	static bool leftMouseButtonDown = false;					// Track left mouse button state
	static int lastMouseX = 0;									// Last mouse X position of the mouse for dragging
	static bool followWrist = false;							// Follow wrist mode

	// Input handling for joint selection and rotation
	// Convert keyboard state to joint selection
	if (GetAsyncKeyState('1') & 0x8000) selectedJoint = 0;
	if (GetAsyncKeyState('2') & 0x8000) selectedJoint = 1;
	if (GetAsyncKeyState('3') & 0x8000) selectedJoint = 2;
	if (GetAsyncKeyState('4') & 0x8000) selectedJoint = 3;
	if (GetAsyncKeyState('5') & 0x8000) selectedJoint = 4;

	// Convert the space bar state to follow wrist mode
	...
	// If left button is down, rotate the selected joint based on mouse movement
	POINT mousePos;
	...
	if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
	{
		...
		{
			// Calculate the horizontal movement of the mouse
			int dx = mousePos.x - lastMouseX;
			// Update the selected joint angle based on mouse movement
			jointAngles[selectedJoint] += dx * 0.5f;
			// Update last mouse X position
			lastMouseX = mousePos.x;
		}
	}
	else
	...
	// Joint 1
	glPushMatrix();
		glRotatef(jointAngles[0], 0, 1, 0);
		glTranslatef(0, 1, 0);
		glutSolidcube(1.0);
	
		// Joint 2
		glPushMatrix();
			glRotatef(jointAngles[1], 0, 0, 1);
			glTranslatef(0, 1, 0);
			glutSolidcube(1.0);

			// Joint 3
			glPushMatrix();
				glRotatef(jointAngles[2], 0, 0, 1);
				glTranslatef(0, 1, 0);
				glutSolidcube(1.0);

				// Joint 4
				glPushMatrix();
					glRotatef(jointAngles[3], 1, 0, 0);
