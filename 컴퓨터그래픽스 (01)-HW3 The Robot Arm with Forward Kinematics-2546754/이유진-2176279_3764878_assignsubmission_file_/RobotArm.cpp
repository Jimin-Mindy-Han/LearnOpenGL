...
bool teapotMove = false; //teapot control 여부 
//glm::mat4 objectXform = glm::scale(glm::rotate(glm::translate(glm::mat4(1.0f), glm::vec3(teapotX, 0.0f, teapotZ)), glm::radians(teapotAng), glm::vec3(0.0f, 0.0f, 1.0f)), glm::vec3(0.08f, 0.08f, 0.08f));
//glm::mat4 objectXform = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(teapotX, 0.0f, teapotZ)), glm::vec3(0.08f, 0.08f, 0.08f));
...
void myDisplay()
{
	...
	//teapot - space bar 구현 위해 수정한 코드 
	glm::mat4 objectXform = glm::mat4(1.0f);
	
	objectXform = glm::rotate(objectXform, glm::radians(teapotSpin), glm::vec3(0.0f, 1.0f, 0.0f));//y축 중심으로 공전 : 2
	objectXform = glm::rotate(objectXform, glm::radians(teapotAng), glm::vec3(0.0f, 0.0f, 1.0f));// 3, 4
	objectXform = glm::rotate(objectXform, glm::radians(teapotTwistAng), glm::vec3(1.0f, 0.0f, 0.0f)); //4
	objectXform = glm::translate(objectXform, glm::vec3(teapotX, 0.0f, teapotZ));  //1
	...
	robotBase = glm::translate(robotBase, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	robotBase = glm::rotate(robotBase, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));

	DrawBase(robotBase);//로봇 베이스: 맨 아래 지지대, 여기 안에 1차 관절까지 포함 

	glm::mat4 shoulder = robotBase;//파란색 관절 - 1
	shoulder = glm::translate(shoulder, glm::vec3(0.0f, 0.4f, 0.0f));
	shoulder = glm::rotate(shoulder, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(shoulder);

	glm::mat4 elbow = shoulder;//파란색 관절 - 2
	...
	glm::mat4 wrist = elbow;//finger 지지대: 빨간색 부분 지지대
	wrist = glm::translate(wrist, glm::vec3(0.0f, 0.5f, 0.0f));
	wrist = glm::rotate(wrist, glm::radians(WristTwistAng), glm::vec3(1.0f, 0.0f, 0.0f));
	wrist = glm::rotate(wrist, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	
	DrawWrist(wrist);

	//finger 2개
	glm::mat4 finger1 = glm::translate(wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	finger1 = glm::rotate(finger1, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(finger1);
	glm::mat4 fingerTip1 = glm::translate(finger1, glm::vec3(0.0f, 0.35f, 0.0f));
	fingerTip1 = glm::rotate(fingerTip1, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(fingerTip1);

	glm::mat4 finger2 = glm::translate(wrist, glm::vec3(0.0f, 0.2f, 0.0f));
	finger2 = glm::rotate(finger2, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(finger2);
	glm::mat4 fingerTip2 = glm::translate(finger2, glm::vec3(0.0f, 0.35f, 0.0f));
	fingerTip2 = glm::rotate(fingerTip2, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(fingerTip2);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)
	{
		...
		case 0: 
			...
			//teapot move 
			teapotX += xoffset* teapotMove;
			teapotZ -= yoffset* teapotMove; 
			break;
		case 1: 
			BaseSpin += xoffset * 180 ; 
			teapotSpin += xoffset * 180 * teapotMove; //teapot spin(y축 공전)
			break;
		case 2: 
			ShoulderAng += yoffset   * -90; ElbowAng += xoffset  * 90; 
			teapotAng += yoffset * -90 * teapotMove; //teapot spin(z축 공전)
			break;
		case 3: 
			WristAng += yoffset  * -180; WristTwistAng += xoffset  * 180; 
			teapotAng += yoffset * -180 * teapotMove; teapotTwistAng += xoffset * 180 * teapotMove;
			break;
		case 4: FingerAng1 += yoffset  * 90; FingerAng2 += xoffset * 180; 
			break;
		}
	} 
