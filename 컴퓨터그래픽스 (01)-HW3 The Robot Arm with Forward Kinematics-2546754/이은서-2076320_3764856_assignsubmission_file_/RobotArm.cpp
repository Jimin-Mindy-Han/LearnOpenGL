...
#include <robotarm/shader_m.h>
#include <robotarm/camera.h>
#include <robotarm/model.h>
...
const char* ourObjectPath = "./assets/teapot.obj";
...
void myDisplay()
{
	...
	// 1) '부모 끝(피벗)'까지 이동:           
	// 2) 그 자리(피벗)에서 회전:              
	// 3) 자식 링크 그릴 때(unit원통: 중앙=0, 높이=1)
	//    - 밑면이 피벗에 딱 닿도록 +0.5L 올림
	//    - 실제 크기 적용: 

	//링크 길이, 반경
	const float L1 = 0.38f, R_base = 0.10f;
	const float L2 = 0.60f, R_j2 = 0.10f;
	const float L3 = 0.43f, R_j3 = 0.085f;
	const float L_4 = 0.28f, R_j4 = 0.070f;

	// 컬러
	auto C = [](float r, float g, float b) { return glm::vec3(r, g, b); };
	const glm::vec3 C_BASE = C(0.60f, 0.60f, 0.60f);
	const glm::vec3 C_J2 = C(0.95f, 0.40f, 0.30f);
	const glm::vec3 C_J3 = C(0.30f, 0.80f, 0.50f);
	const glm::vec3 C_J4 = C(0.30f, 0.40f, 0.90f);

	// 축:직교축
	const glm::vec3 AX_X = { 1,0,0 }; // 굴곡
	const glm::vec3 AX_Y = { 0,1,0 }; // 비틀림

	// 전역 각도
	float angle1 = BaseSpin;        
	float angle2 = ShoulderAng;     
	float angle3 = ElbowAng;        
	float angle4 = WristAng;        
	float angle5 = WristTwistAng;   

	// ----- 단위원통을 '밑면=피벗'으로 보정 후 그리는 헬퍼 -----
	auto DrawLink = [&](const glm::mat4& parentPivot, float R, float L, const glm::vec3& color)
		{
			glm::mat4 localShape =
				glm::translate(I, glm::vec3(0.0f, 0.5f * L, 0.0f))
				* glm::scale(I, glm::vec3(R, L, R));

			// ── ② 메쉬/셰이더 직접 제어형 (요청 반영) ───────────────────────────────
			// 주의: PhongShader/unitCylinder 는 프로젝트에 실제 존재하는 객체여야 함.
			PhongShader->use();
			PhongShader->setVec3("ObjColor", color);
			PhongShader->setMat4("model", pivot * localShape);
			unitCylinder->Draw();
		};

	// (F) 피벗들 p1..p4  ─────────────────────────────────────────────────────────
	// p1: 살짝 띄우고(Y 회전)
	glm::mat4 p1 = glm::translate(I, glm::vec3(0.0f, 0.06f, 0.0f))               // [MODIFIED: 이름 p1]
		* glm::rotate(I, glm::radians(angle1), AXIS_Y);
	DrawLink(p1, R1, L1, COL1);

	// p2: p1 끝까지 → X 회전
	glm::mat4 p2 = p1
		* glm::translate(I, glm::vec3(0.0f, L1, 0.0f))
		* glm::rotate(I, glm::radians(angle2), AXIS_X);
	DrawLink(p2, R2, L2, COL2);

	// p3: p2 끝까지 → X 회전
	glm::mat4 p3 = p2
		* glm::translate(I, glm::vec3(0.0f, L2, 0.0f))
		* glm::rotate(I, glm::radians(angle3), AXIS_X);
	DrawLink(p3, R3, L3, COL3);

	// p4: p3 끝까지 → X 굴곡 + Y 비틀림
	glm::mat4 p4 = p3
		* glm::translate(I, glm::vec3(0.0f, L3, 0.0f))
		* glm::rotate(I, glm::radians(angle4), AXIS_X)
		* glm::rotate(I, glm::radians(angle5), AXIS_Y);
	DrawLink(p4, R4, L4, COL4);

	// ── Extra credit: 티팟 팔끝 따라가기 토글 ────────────────────────────────────
	extern bool g_FollowTeapot;                                           // [ADDED] 전역 토글(키 콜백에서 Space로 반전)

	glm::mat4 teapotOnWrist =
		p4
		* glm::translate(I, glm::vec3(0.0f, 0.5f * L4, 0.0f))             // 손목 끝 기준
		* glm::translate(I, glm::vec3(0.0f, 0.08f, 0.10f))               // 보기 좋게 오프셋
		* glm::scale(I, glm::vec3(0.50f));                            // 크기 조정

	if (g_FollowTeapot)                                                   // [ADDED]
		DrawObject(teapotOnWrist);                                        // 팔끝 모드
	else
		DrawObject(objectXform);                                          // 기존 자유 배치 모드
...
void setupShader()
{
	PhongShader = new Shader("shaders/model_loading.vs", "shaders/model_loading.fs");
	PhongShader->use();

	// Light attributes
	PhongShader->setVec3("lightColor", lightColor);

	FloorShader = new Shader("shaders/advanced_lighting.vs", "shaders/advanced_lighting.fs");
	FloorShader->use();
...
Plane::Plane()
{
	...
	floorTexture = loadTexture("./assets/wood.png");
	FloorShader->use();
