...
void myDisplay()
{
	...
	DrawBase(Base);
	glm::mat4 Arm1 = glm::translate(Base, glm::vec3(0.0f, 0.4f, 0.0f));
	Arm1 = glm::rotate(Arm1, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(Arm1);
	glm::mat4 Arm2 = glm::translate(Arm1, glm::vec3(0.0f, 0.5f, 0.0f));
	Arm2 = glm::rotate(Arm2, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(Arm2);
	glm::mat4 Wrist = glm::translate(Arm2, glm::vec3(0.0f, 0.5f, 0.0f));
	...
	glm::mat4 Teapot = glm::translate(Wrist, glm::vec3(TeapotTransX, 0.0f, TeapotTransZ));
	Teapot = glm::rotate(Teapot, glm::radians(45.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	Teapot = glm::scale(Teapot, glm::vec3(0.08f));
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
		...
		followWristjoint = !followWristjoint;
}
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)
	{
		...
		case 3: WristAng += yoffset * -180; WristTwistAng += xoffset * 180; TeapotTransX += xoffset; TeapotTransZ -= yoffset; break;
