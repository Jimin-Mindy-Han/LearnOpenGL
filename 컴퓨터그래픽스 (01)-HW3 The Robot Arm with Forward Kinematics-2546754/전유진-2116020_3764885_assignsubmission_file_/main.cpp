 ...
 * \file   main.cpp
 * \brief  
 * extra credit: 티팟이 손목위치 따라다니는 것도 구현했습니다.(+반경거리에 따라 집히는지 판단)
 * \author 전유진
 * \date   October 2025
 *********************************************************************/
#include <glad/glad.h>
...
// 티팟 컨트롤
bool folloWrist = false;
const float minimumDistance = 0.35f; //손목위치 따라다니기 위한 최소 반경거리
...
void myDisplay()
{
	...
	//로봇팔이 할 것: 모델링 트랜스폼
	//모델링 트랜스폼에서 이동, 회전시키기. (렌더링(보이는모습)에 영향)
	//x축 회전: 상하, y축회전: 좌우, z축회전: 앞뒤(시계방향)
	
	//베이스
	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	model = glm::rotate(model, glm::radians(BaseSpin), glm::vec3(0, 1, 0));
	DrawBase(model);

	//어깨
	model = glm::translate(model, glm::vec3(0, 0.425f, 0));
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0, 0, 1));
	DrawArmSegment(model);

	//팔꿈치
	model = glm::translate(model, glm::vec3(0, 0.5f, 0));
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(0, 0, 1));
	DrawArmSegment(model);

	//손목
	model = glm::translate(model, glm::vec3(0, 0.5f, 0));
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(0, 0, 1));
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0, 1, 0));	
	DrawWrist(model);

	//손가락1(위)
	model = glm::translate(model, glm::vec3(0, 0.2f, 0));
	glm::mat4 wristBase = model;
	model = glm::rotate(wristBase, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	glm::mat4 upFingerTipBase = glm::translate(model, glm::vec3(0, 0.35f, 0));
	DrawFingerBase(model);

	//손가락1(아래)
	model = glm::rotate(wristBase, glm::radians(FingerAng1*-1), glm::vec3(0, 0, 1));
	glm::mat4 downFingerTipBase = glm::translate(model, glm::vec3(0, 0.35f, 0));
	DrawFingerBase(model);
	
	//손가락2(위)
	model = glm::rotate(upFingerTipBase, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(model);

	//손가락2(아래)
	model = glm::rotate(downFingerTipBase, glm::radians(FingerAng2*-1), glm::vec3(0, 0, 1));
	DrawFingerTip(model);

	//티팟이 손목위치 따라가기
	if(folloWrist){
		//조건검사: 반경거리 내에 있을 때만 손목위치 따라다니기
		glm::mat4 teapotBase = wristBase;
		glm::mat4 fingertipPos = glm::translate(wristBase, glm::vec3(0.0f, 0.3f, 0.0f));
		//fingertipPos에서 반경거리 재기 ([3]: 위치벡터)
		float distance = glm::length(objectXform[3] - fingertipPos[3]);
		//만약 원 안에 티팟 Pos이 있으면, 손목위치 따라다니기
		if(distance < minimumDistance){
		teapotBase = glm::translate(teapotBase, glm::vec3(0.0f,0.3f,0.0f));
			teapotBase = glm::scale(teapotBase, glm::vec3(0.08f,0.08f,0.08f));
			DrawObject(teapotBase);
			objectXform = teapotBase;
		}
	}else{
		DrawObject(objectXform);
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	//스페이스 누르는 여부에 따라 티팟이 wrist위치 따라갈지 말지
	else if(key == GLFW_KEY_SPACE && action == GLFW_PRESS){
		folloWrist = !folloWrist;
	}
}
