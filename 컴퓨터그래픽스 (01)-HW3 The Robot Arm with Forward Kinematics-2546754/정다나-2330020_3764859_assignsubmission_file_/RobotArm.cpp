...
void myDisplay()
{
	...
	DrawGroundPlane(model); //floor

	//DrawObject(objectXform); //teapot
	...
	model = glm::rotate(model, glm::radians(BaseSpin), y);
	DrawBase(model);

	model = glm::rotate(glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f)), glm::radians(ShoulderAng), z);
	DrawArmSegment(model);

	model = glm::rotate(glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f)), glm::radians(ElbowAng), z);
	DrawArmSegment(model);

	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, glm::radians(WristAng), z);
	model = glm::rotate(model, glm::radians(WristTwistAng), y);
	DrawWrist(model);

	if (toggle) {
		teapot = model * relative;
		DrawObject(teapot);
	}
	else {
		DrawObject(teapot);
		relative = glm::inverse(model) * teapot;
	}
	
	model = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f));
	glm::mat4 model1 = glm::rotate(model, glm::radians(FingerAng1), z);
	glm::mat4 model2 = glm::rotate(model, glm::radians(-FingerAng1), z);
	DrawFingerBase(model1); DrawFingerBase(model2);

	model1 = glm::rotate(glm::translate(model1, glm::vec3(0.0f, 0.35f, 0.0f)), glm::radians(FingerAng2), z);
	model2 = glm::rotate(glm::translate(model2, glm::vec3(0.0f, 0.35f, 0.0f)), glm::radians(-FingerAng2), z);
	DrawFingerTip(model1); DrawFingerTip(model2);
