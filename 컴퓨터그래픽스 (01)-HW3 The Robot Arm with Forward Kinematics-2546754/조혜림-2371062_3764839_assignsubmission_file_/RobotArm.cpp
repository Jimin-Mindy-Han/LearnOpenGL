...
void myDisplay()
{
	...
	// ¸ðµ¨ µ¿ÀÛ°ú °°ÀÌ ¿òÁ÷ÀÌµµ·Ï DrawObject·Î objectXform ±×¸®±â
	if (TeapotControl == 1) {
		// glm::mat4 objectXform = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)), glm::vec3(0.08f, 0.08f, 0.08f));
		// objectXform¿Í µ¿ÀÏÇÑ teapot »ý¼º
		glm::mat4 teapot = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)), glm::vec3(0.08f, 0.08f, 0.08f));

		// ¼öÁ¤ ÇÊ¿ä
		objectXform = glm::translate(teapot, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
		objectXform = glm::rotate(objectXform, BaseSpin * 0.01f, glm::vec3(0.0f, 1.0f, 0.0f));
	...
	// ¸ðµ¨1: ¹Ù´Ú¿¡ DrawBase·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model1 = glm::mat4(1.0f);
	model1 = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	model1 = glm::rotate(model1, BaseSpin*0.01f, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(model1);

	// ¸ðµ¨2: ¸ðµ¨1ÀÇ Joint¿¡ DrawArmSegment·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model2 = glm::mat4(1.0f);
	model2 = glm::translate(model1, glm::vec3(0.0f, 0.4f, 0.0f));
	model2 = glm::rotate(model2, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model2);

	// ¸ðµ¨3: ¸ðµ¨2ÀÇ Joint¿¡ DrawArmSegment·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model3 = glm::mat4(1.0f);
	model3 = glm::translate(model2, glm::vec3(0.0f, 0.5f, 0.0f));
	model3 = glm::rotate(model3, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model3);

	// ¸ðµ¨4: ¸ðµ¨3ÀÇ Joint¿¡ DrawWrist·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model4 = glm::mat4(1.0f);
	model4 = glm::translate(model3, glm::vec3(0.0f, 0.5f, 0.0f));
	model4 = glm::rotate(model4, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	model4 = glm::rotate(model4, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(model4);

	// ¸ðµ¨5-A: ¸ðµ¨4ÀÇ Wrist¿¡ DrawFingerBase·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model5_A = glm::mat4(1.0f);
	model5_A = glm::translate(model4, glm::vec3(0.0f, 0.2f, 0.0f));
	model5_A = glm::rotate(model5_A, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(model5_A);

	// ¸ðµ¨6-A: ¸ðµ¨5-AÀÇ FingerJoint¿¡ DrawFingerTipÀ¸·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model6_A = glm::mat4(1.0f);
	model6_A = glm::translate(model5_A, glm::vec3(0.0f, 0.35f, 0.0f));
	model6_A = glm::rotate(model6_A, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(model6_A);
	
	// ¸ðµ¨5-B: ¸ðµ¨4ÀÇ Wrist¿¡ DrawFingerBase·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model5_B = glm::mat4(1.0f);
	model5_B = glm::translate(model4, glm::vec3(0.0f, 0.2f, 0.0f));
	model5_B = glm::rotate(model5_B, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, -1.0f));
	DrawFingerBase(model5_B);

	// ¸ðµ¨6-B: ¸ðµ¨5-BÀÇ FingerJoint¿¡ DrawFingerTipÀ¸·Î ¸ðµ¨ ±×¸®±â
	glm::mat4 model6_B = glm::mat4(1.0f);
	model6_B = glm::translate(model5_B, glm::vec3(0.0f, 0.35f, 0.0f));
	model6_B = glm::rotate(model6_B, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, -1.0f));
	DrawFingerTip(model6_B);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// ½ºÆäÀÌ½º¹Ù Ãß°¡
		...
		TeapotControl = (TeapotControl + 1) % 2;
	}
