...
void myDisplay()
{
	...
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(1.0f, 0.0f, 0.0f));
	...
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(1.0f, 0.0f, 0.0f));
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(model);

	glm::mat4 finger_pivot = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f));
	{
		glm::mat4 left_arm = glm::rotate(finger_pivot, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
		DrawFingerBase(left_arm);

		glm::mat4 left_tip_pivot = glm::translate(left_arm, glm::vec3(0.0f, 0.35f, 0.0f));
		left_tip_pivot = glm::rotate(left_tip_pivot, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		DrawFingerTip(left_tip_pivot);
	}

	{
		glm::mat4 right_arm = glm::rotate(finger_pivot, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, -1.0f));
		DrawFingerBase(right_arm);

		glm::mat4 right_tip_pivot = glm::translate(right_arm, glm::vec3(0.0f, 0.35f, 0.0f));
		right_tip_pivot = glm::rotate(right_tip_pivot, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, -1.0f));
		DrawFingerTip(right_tip_pivot);
	}

	// Extra Credit
	if (teapotFollowsWrist)
	{
		if (!isAttached)
		{
			teapotOffsetMatrix = glm::inverse(finger_pivot) * objectXform;
			isAttached = true;
		}
		objectXform = finger_pivot * teapotOffsetMatrix;
	}
	
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		teapotFollowsWrist = !teapotFollowsWrist;
		isAttached = false;
	}
}
