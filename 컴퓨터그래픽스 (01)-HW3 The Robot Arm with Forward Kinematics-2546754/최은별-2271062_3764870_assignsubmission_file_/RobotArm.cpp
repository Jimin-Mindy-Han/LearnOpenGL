...
// 주전자 관련 저장
glm::mat4 modelTeapotOff = objectXform;
glm::mat4 modelTeapotOn = objectXform;
...
void myDisplay()
{
	...
	model = glm::rotate(model, glm::radians(ShoulderAng + ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	// Wrist
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, glm::radians(WristAng + WristTwistAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawWrist(model);
	// 주전자
	if (SpacebarOn) { // 토글 켜짐 - wrist 따라 움직임
		modelTeapotOn = model * TeapotOnWrist;
		DrawObject(modelTeapotOn);
	}
	else { // 꺼짐 - 그 자리 고정
		DrawObject(modelTeapotOff);
	}
	// Finger Base 1
	model = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f)); // Finger Base 2에서 돌아갈 자리
	modelFingerBase = model;
	model = glm::rotate(model, glm::radians(FingerAng1 + FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	DrawFingerTip(model);
	// Finger Base 2
	modelFingerBase = glm::rotate(modelFingerBase, glm::radians(FingerAng1 + FingerAng2), glm::vec3(0.0f, 0.0f, -1.0f));
	modelFingerBase = glm::rotate(modelFingerBase, 1.7f, glm::vec3(0.0f, 0.0f, -1.0f));
	DrawFingerBase(modelFingerBase);
	// Finger 2
	modelFingerBase = glm::translate(modelFingerBase, glm::vec3(0.0f, 0.35f, 0.0f));
	modelFingerBase = glm::rotate(modelFingerBase, 1.7f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(modelFingerBase);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// Spacebar 토글
		...
		if (SpacebarOn) { // 끌 때
			SpacebarOn = false;
			modelTeapotOff = modelTeapotOn; // 현 위치 저장
		}
		else { // 킬 때
			SpacebarOn = true;
			// Wrist를 원점으로 보고 주전자 좌표 설정해야
			glm::mat4 modelWrist = glm::mat4(1.0f);
			modelWrist = glm::translate(modelWrist, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
			modelWrist = glm::rotate(modelWrist, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
			modelWrist = glm::translate(modelWrist, glm::vec3(0.0f, 0.4f, 0.0f));
			modelWrist = glm::rotate(modelWrist, glm::radians(ShoulderAng + ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
			modelWrist = glm::rotate(modelWrist, 2.0f, glm::vec3(0.0f, 0.0f, 1.0f));
			modelWrist = glm::translate(modelWrist, glm::vec3(0.0f, 0.5f, 0.0f));
			modelWrist = glm::rotate(modelWrist, 2.0f, glm::vec3(0.0f, 0.0f, -1.0f));
			modelWrist = glm::translate(modelWrist, glm::vec3(0.0f, 0.5f, 0.0f));
			modelWrist = glm::rotate(modelWrist, glm::radians(WristAng + WristTwistAng), glm::vec3(0.0f, 0.0f, 1.0f));

			TeapotOnWrist = glm::inverse(modelWrist) * modelTeapotOff; 
		}
	}
