...
bool teapotMovement = false;
glm::mat4 teapotMatrix;
bool isTeapotCalculating = false;
...
//Extra credit Globals
glm::mat4 currentTeapotXform = objectXform;
...
void myDisplay()
{
	...
	// robot arm's base, shoulder, elbow, wrist
	// 이후 역행렬 계산을 위해 개별 행렬 분리.
	glm::mat4 baseTrans = glm::translate(glm::mat4(1.0f), glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	glm::mat4 baseSpin = glm::rotate(glm::mat4(1.0f), glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	glm::mat4 base = baseTrans * baseSpin;
	DrawBase(base);

	glm::mat4 shoulderTrans = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.4f, 0.0f));
	glm::mat4 shoulderRot = glm::rotate(glm::mat4(1.0f), glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 shoulder = base * shoulderTrans * shoulderRot;
	DrawArmSegment(shoulder);

	glm::mat4 elbowTrans = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.5f, 0.0f));
	glm::mat4 elbowRot = glm::rotate(glm::mat4(1.0f), glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 Elbow = shoulder * elbowTrans * elbowRot;
	DrawArmSegment(Elbow);

	glm::mat4 wristTrans = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.5f, 0.0f));
	glm::mat4 wristRot = glm::rotate(glm::mat4(1.0f), glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 wristTwist = glm::rotate(glm::mat4(1.0f), glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	glm::mat4 Wrist = Elbow * wristTrans * wristRot * wristTwist;
	DrawWrist(Wrist);

	// robot arm's left finger
	glm::mat4 left = Wrist;
	left = glm::translate(left, glm::vec3(0.0f, 0.2f, 0.0f));
	left = glm::rotate(left, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(left);

	glm::mat4 leftFinger = left;
	...
	// robot arm's right finger
	glm::mat4 right = Wrist;
	right = glm::translate(right, glm::vec3(0.0f, 0.2f, 0.0f));
	right = glm::rotate(right, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(right);

	glm::mat4 rightFinger = right;
	rightFinger = glm::translate(rightFinger, glm::vec3(0.0f, 0.35f, 0.0f));
	rightFinger = glm::rotate(rightFinger, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(rightFinger);

	// Extra-credit
	// space 
	if (teapotMovement) {
		// teapotMovement : space bar을 누른 상태
		if (!isTeapotCalculating) {

			// 각 개별 행렬의 역행렬 정의
			// rotate -> transpose
			// translation -> -translation
			glm::mat4 invWristTwist = glm::transpose(wristTwist);
			glm::mat4 invWristRot = glm::transpose(wristRot);
			glm::mat4 invWristTrans = glm::translate(glm::mat4(1.0f), -glm::vec3(0.0f, 0.5f, 0.0f));

			glm::mat4 invElbowRot = glm::transpose(elbowRot);
			glm::mat4 invElbowTrans = glm::translate(glm::mat4(1.0f), -glm::vec3(0.0f, 0.5f, 0.0f));

			glm::mat4 invShoulderRot = glm::transpose(shoulderRot);
			glm::mat4 invShoulderTrans = glm::translate(glm::mat4(1.0f), -glm::vec3(0.0f, 0.4f, 0.0f));

			glm::mat4 invBaseSpin = glm::transpose(baseSpin);
			glm::mat4 invBaseTrans = glm::translate(glm::mat4(1.0f), -glm::vec3(BaseTransX, 0.0f, BaseTransZ));
			
			// 역행렬 사용
			teapotMatrix = invWristTwist * invWristRot * invWristTrans *
							invElbowRot * invElbowTrans *
							invShoulderRot * invShoulderTrans *
							invBaseSpin * invBaseTrans * currentTeapotXform;

			isTeapotCalculating = true;
		}

		currentTeapotXform = Wrist * teapotMatrix;
	}
	DrawObject(currentTeapotXform);
