...
void myDisplay()
{
	...
	DrawGroundPlane(model); //바닥 

	DrawObject(objectXform);
	// 1) BASE 
	glm::mat4 M_base = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	M_base = glm::rotate(M_base, glm::radians(BaseSpin), glm::vec3(0, 1, 0));
	DrawBase(M_base);                    

	// 2) SHOULDER 
	glm::mat4 M_shoulder = glm::translate(M_base, glm::vec3(0.0f, 0.4f, 0.0f));
	M_shoulder = glm::rotate(M_shoulder, glm::radians(ShoulderAng), glm::vec3(0, 0, 1));
	DrawArmSegment(M_shoulder);         

	// 3) ELBOW 
	glm::mat4 M_elbow = glm::translate(M_shoulder, glm::vec3(0.0f, 0.5f, 0.0f));
	M_elbow = glm::rotate(M_elbow, glm::radians(ElbowAng), glm::vec3(0, 0, 1));
	DrawArmSegment(M_elbow); 

	// 4) WRIST
	glm::mat4 M_wristJoint = glm::translate(M_elbow, glm::vec3(0.0f, 0.5f, 0.0f));
	glm::mat4 M_wrist = M_wristJoint;
	M_wrist = glm::rotate(M_wrist, glm::radians(WristAng), glm::vec3(0, 0, 1));
	M_wrist = glm::rotate(M_wrist, glm::radians(WristTwistAng), glm::vec3(1, 0, 0));
	DrawWrist(M_wrist);

	// 5) FINGERS
	const float ATTACH_Y = 0.150f;

	// 왼쪽(수직)
	glm::mat4 L = glm::translate(M_wrist, glm::vec3(0.0f, ATTACH_Y, 0.0f));
	L = glm::rotate(L, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(L);

	glm::mat4 L_tip = glm::translate(L, glm::vec3(0.0f, 0.35f, 0.0f));
	L_tip = glm::rotate(L_tip, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(L_tip);

	// 오른쪽(가로) 
	constexpr float RIGHT_DELTA_PRE = 0.06f;   

	// 회전
	glm::mat4 R = glm::translate(M_wrist, glm::vec3(0.0f, ATTACH_Y + RIGHT_DELTA_PRE, 0.0f));
	R = glm::rotate(R, glm::radians(-90.0f), glm::vec3(0, 0, 1));

	constexpr float RIGHT_DELTA_POST = -0.02f;  
	R = glm::translate(R, glm::vec3(0.0f, RIGHT_DELTA_POST, 0.0f));

	// 굽힘(왼쪽과 부호 반대)
	R = glm::rotate(R, -glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(R);

	glm::mat4 R_tip = glm::translate(R, glm::vec3(0.0f, 0.35f, 0.0f));
	R_tip = glm::rotate(R_tip, -glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(R_tip);
	
