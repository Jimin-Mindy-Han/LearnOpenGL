#include <glad/glad.h> // include glad lib
...
void myDisplay()
{
	glClearColor(0.1f, 0.1f, 0.1f, 1.0f); //색깔 clear
	...
	DrawGroundPlane(model); //draw the ground - DrawGroundPlane() 참고 하기

	DrawObject(objectXform); //draw the teapot
	

	// ADD YOUR ROBOT RENDERING STUFF HERE     /////////////////////////////////////////////////////

	glm::mat4 base = model;
	base = glm::translate(base, glm::vec3(BaseTransX,0,0));
	base = glm::translate(base, glm::vec3(0,0,BaseTransZ));
	base = glm::rotate(base, glm::radians(BaseSpin), glm::vec3(0,1,0));
	DrawBase(base);


	glm::mat4 arm1 = base;
	arm1 = glm::translate(arm1, glm::vec3(0.0f, 0.4f, 0.0f)); // Base 위 Joint 위치를 새로 좌표계 설정
	arm1 = glm::rotate(arm1, glm::radians(ShoulderAng), glm::vec3(0, 0, 1)); //z축으로 shoulderAng 만큼 회전

	DrawArmSegment(arm1); //arm1 좌표계를 기준으로 팔을 하나 그림

	glm::mat4 arm2 = arm1;
	arm2 = glm::translate(arm2, glm::vec3(0.0f, 0.5f, 0.0f)); // arm1 좌표계 위에다가 DrawArmSegment()의 Joint가 그려지는 위치만큼 y축을 추가해서 arm2 좌표계를 설정함
	arm2 = glm::rotate(arm2, glm::radians(ElbowAng), glm::vec3(0, 0, 1)); //z축으로 ElbowAng만큼 회전
	DrawArmSegment(arm2);

	glm::mat4 wrist = arm2;
	wrist = glm::translate(wrist, glm::vec3(0.0f, 0.5f, 0.0f)); //arm2 좌표계 위에다 y축으로의 값을 추가하여 wrist좌표계 설정함
	...
	glm::mat4 FingerBase = wrist;
	FingerBase = glm::translate(FingerBase, glm::vec3(0.0f, 0.2f, 0.0f));
	FingerBase = glm::rotate(FingerBase, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(FingerBase);

	glm::mat4 FingerBase2 = wrist; 
	FingerBase2 = glm::translate(FingerBase2, glm::vec3(0.0f, 0.2f, 0.0f));
	FingerBase2 = glm::rotate(FingerBase2, glm::radians(-(FingerAng1)), glm::vec3(0, 0, 1));
	DrawFingerBase(FingerBase2); 

	
	glm::mat4 FingerL = FingerBase;
	FingerL = glm::translate(FingerL, glm::vec3(0.0f, 0.35f, 0.0f));
	FingerL = glm::rotate(FingerL, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(FingerL);

	glm::mat4 FingerR = FingerBase2;
	FingerR = glm::translate(FingerR, glm::vec3(0.0f, 0.35f, 0.0f));
	FingerR = glm::rotate(FingerR, glm::radians(-(FingerAng2)), glm::vec3(0, 0, 1));
	DrawFingerTip(FingerR);
...
void createGLPrimitives()
{
	...
	// Load Object Model
	ourObjectModel = new Model(ourObjectPath); //여기서 '티팟 오브젝트' 로드
