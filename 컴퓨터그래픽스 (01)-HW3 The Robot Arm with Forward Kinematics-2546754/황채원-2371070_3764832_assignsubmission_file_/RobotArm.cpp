...
bool isTeapotTransformed = false;
glm::mat4 teapotTransformMtx;
...
void CalObjectForm(glm::mat4 model) {
	if (isPressedSpace) {
		if (!isTeapotTransformed) {
			teapotTransformMtx = glm::inverse(model) * objectXform;  // model의 좌표계 기준 object의 위치 matrix 계산 
			isTeapotTransformed = true;
		}
		objectXform = model * teapotTransformMtx; // object의 위치를 (model * model 좌표계 기준 object위치)로 바꾸기.
								// 즉, space 누른 시점의 model 좌표계 기준 object 위치를 유지하면서 model이 움직일 때 같이 움직이도록 함.
	}
	else {
		isTeapotTransformed = false;
	}
}
...
void myDisplay()
{
	...
	glm::mat4 temp_model = glm::mat4(1.0f); // 로봇 손가락 기준 위치 저장 
	glm::mat4 object_temp_model = glm::mat4(1.0f); // 오브젝트(teapot) 기준 위치 저장

	DrawGroundPlane(model);

	if (!isPressedSpace || RobotControl == 4) {
	...
	model = glm::rotate(model, glm::radians(FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(model);

	model = glm::translate(model, glm::vec3(0, 0.35f, 0));
	model = glm::rotate(model, glm::radians(FingerAng2), glm::vec3(0, 0, 1));
	DrawFingerTip(model);

	model = glm::translate(temp_model, glm::vec3(0, 0.2f, 0));
	model = glm::rotate(model, glm::radians(-FingerAng1), glm::vec3(0, 0, 1));
	DrawFingerBase(model);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)
	{
		...
		case 1: 
			...
			break;
		case 2: 
			ShoulderAng += yoffset   * -90; 
			ElbowAng += xoffset  * 90;
			break;
		case 3: 
			WristAng += yoffset  * -180; 
			WristTwistAng += xoffset  * 180;
			break;
		case 4: 
			FingerAng1 += yoffset  * 90; 
			FingerAng2 += xoffset * 180;
			break;
		}
	} 
}
